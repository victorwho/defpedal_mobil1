...
- [ ] NOT STARTED — **Anonymous → real account upgrade (preserve user id + ride data).** Today `signUpWithEmail()` and `signInWithGoogle()` call Supabase `auth.signUp()` / `auth.signInWithOAuth()`, which create a **new** Supabase user and orphan everything tied to the anonymous user (early trips, hazards, votes, XP, badges, streak, Mia journey state). Switch the *anonymous-user* path to `supabaseClient.auth.updateUser({ email, password })` for email/password sign-up and `supabaseClient.auth.linkIdentity({ provider: 'google' })` for OAuth — both preserve the underlying `auth.users.id`. Keep the current `signUp` / `signInWithOAuth` calls as the fallback for fully-signed-out users. After the change, `UserCacheResetBridge` sees the same user id throughout the upgrade → no Zustand reset, no TanStack cache wipe, no router redirect. Files: `apps/mobile/src/lib/supabase.ts` (branching in `signUpWithEmail` / `signInWithGoogle`), one or two unit tests in `apps/mobile/src/lib/__tests__/supabase.test.ts`. Identified 2026-04-25 while fixing the anonymous Profile UX (changes already shipped: friendlier Profile CTA, auth.tsx no longer treats anon as signed-in, `resetUserScopedState` no longer clears `onboardingCompleted`).
