...
- [ ] NOT STARTED — **Anonymous → real account upgrade (preserve user id + ride data).** Today `signUpWithEmail()` calls Supabase `auth.signUp()` and `signInWithGoogle()` calls `auth.signInWithIdToken()` (native Google Sign-In as of 2026-05-21 — was `auth.signInWithOAuth()`), both of which create a **new** Supabase user and orphan everything tied to the anonymous user (early trips, hazards, votes, XP, badges, streak). Switch the *anonymous-user* path to `supabaseClient.auth.updateUser({ email, password })` for email/password sign-up to preserve the underlying `auth.users.id`. **NOTE (native sign-in):** the Google path is no longer a browser OAuth flow, so `auth.linkIdentity({ provider: 'google' })` (which opens a browser) no longer matches the UX — investigate whether Supabase supports linking an anonymous user via the native ID token (`signInWithIdToken` does not link by default; it switches/creates a session). May need a server-side link or a `linkIdentity` exception for the Google case. Keep the current `signUp` / `signInWithIdToken` calls as the fallback for fully-signed-out users. After the change, `UserCacheResetBridge` sees the same user id throughout the upgrade → no Zustand reset, no TanStack cache wipe, no router redirect. Files: `apps/mobile/src/lib/supabase.ts` (branching in `signUpWithEmail` / `signInWithGoogle`), one or two unit tests in `apps/mobile/src/lib/__tests__/supabase.test.ts`. Identified 2026-04-25 while fixing the anonymous Profile UX (changes already shipped: friendlier Profile CTA, auth.tsx no longer treats anon as signed-in, `resetUserScopedState` no longer clears `onboardingCompleted`).
...
- [ ] NOT STARTED — Force `com.google.android.material:material:1.13+` in `apps/mobile/android/app/build.gradle` dependencies block. Material 1.13 migrates `EdgeToEdgeUtils` / `BottomSheetDialog.onCreate` / `SheetDialog.onCreate` to the new APIs. Quick win that doesn't require an RN upgrade. Verify androidx pin compatibility against current RN 0.83 before adding (test on dev build first, then preview build).
