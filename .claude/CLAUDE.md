...
- **Sentry does NOT deobfuscate Java/Kotlin frames yet** — no Sentry Android Gradle plugin is applied and `mapping.txt` is not uploaded. Play Console / Android Vitals deobfuscate automatically from the mapping AGP embeds in the AAB. Hermes frames are unaffected. Retrace locally with `mapping.txt` from `C:/dpb/apps/mobile/android/app/build/outputs/mapping/<flavor>Release/` if a native trace matters.
...
### Known Incomplete
- **Production AAB with R8 NOT yet built or uploaded (2026-09-10).** The shrunk config is device-confirmed only on preview v0.2.156 (159). Next: `npm run build:production` from a clean `C:/dev/defpedal` at `origin/main`, verify signer + the R8 keep check in the log, archive to `apkreleases/`, upload to Internal testing, and confirm App bundle explorer "App optimisation" moved off Low before promoting. Sentry `mapping.txt` upload is also outstanding (see the Android R8 section).
