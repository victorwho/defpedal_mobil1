...
## Android R8 (code shrinking) — on since 2026-09-09, device-confirmed on preview v0.2.156 (2026-09-10)

- **Why:** Play Console rated production bundle 133 (v0.2.130) "App optimisation: Low" — obfuscation 2%, 48.5 MB uncompressed DEX. `minifyEnabled` had resolved to `false` for the app's entire history because `android.enableMinifyInReleaseBuilds` was never set anywhere. Now `true` in `apps/mobile/android/gradle.properties`, with resource shrinking, `proguard-android-optimize.txt`, and **R8 full mode deliberately OFF** for the first shipped minified release.
- ⚠️ **`app/proguard-rules.pro` is load-bearing and easy to lose.** The whole `android` tree is gitignored, so the file is force-added (`git add -f`) — an edit that is not re-added is invisible to every other checkout. It is ALSO on the sync list in `scripts/build-preview.sh`; it was absent from that list AND absent from `C:\dpb` entirely when R8 was first switched on.
- ⚠️ **Expo modules are kept wholesale (`-keep class expo.modules.** { *; }`) and so is kotlin-reflect.** expo-modules-core's own rules keep the `Record` classes but NOT the `@Field` annotation kotlin-reflect reads them through; R8 renamed it and the first Record conversion — the Supabase session read on cold start — threw, which `AuthSessionProvider` treats as an expired token: every rider was signed out (error-log #116). Twenty-one Expo modules in this app share that path. Measured cost: uncompressed DEX 15.6 MB -> 18.2 MB (48.5 MB unshrunk). Tighten only with a device pass per step.
- **Verify by content, in the build.** `scripts/check-r8-keeps.sh` reads R8's `mapping.txt` / `usage.txt` after every minified build (build-preview.sh step 3b) and fails the build if the annotation classes are renamed or anything is removed under `expo.modules.` / `kotlin.reflect.jvm.internal.`. R8 failures are runtime-only and release-only; typecheck, tests and the bundle-content check cannot see them. When a tester reports a feature broken on a shrunk build, pull the Sentry event for THAT release first, and build the same source with `-Pandroid.enableMinifyInReleaseBuilds=false` (a ~2.5 min incremental build) as the control.
- **Sentry symbolication for Android builds is wired by hand in `app/build.gradle` (2026-09-10) — and had NEVER worked before.** Two hooks, exactly what `expo prebuild` would have injected: `@sentry/react-native`'s `sentry.gradle.kts` (JS bundle + source map, hooked onto the RN bundle task) and the Sentry Android Gradle Plugin `io.sentry.android.gradle` 6.15.0 via the `plugins {}` DSL (R8 `mapping.txt`, uploaded under a UUID it also writes into `assets/sentry-debug-meta.properties`; native-symbol upload OFF, `autoInstallation` OFF so it cannot pull a second `sentry-android`). Credentials come ONLY from the environment: `build-preview.sh` step 2c exports `SENTRY_AUTH_TOKEN` / `SENTRY_ORG` / `SENTRY_PROJECT` from `apps/mobile/.env`; production fails there if any is missing, other flavors warn and set `SENTRY_DISABLE_AUTO_UPLOAD=true`. Step 3c checks by content that the UUID asset is inside the artefact (an upload whose UUID is not in the build symbolicates nothing and looks fine in the log). First verified uploads: production v0.2.156 (159) — proguard UUID `d6ce1da6-93d1-302c-a7f6-35d1db41b9cf`, 78 MB, and an artifact bundle for release `com.defensivepedal.mobile@0.2.156+159` dist 159. Before that date the runbook's "set SENTRY_AUTH_TOKEN in your shell" advice was read by nothing, so every Android crash frame arrived as `index.android` / obfuscated Java (`docs/ops/sentry-setup.md`, History note). Play Console / Android Vitals deobfuscate independently from the mapping AGP embeds in the AAB.
- **AGP is 8.12.0**, pinned by react-native 0.83's version catalog; Play's "upgrade to AGP 9" suggestion is not reachable without moving off RN 0.83. Dropping x86 ABIs from production is already settled as unnecessary (AAB, Play splits per device — see build-preview.sh).
...
- ⚠️ **`gcloud builds submit` uploads the WORKING DIRECTORY, not the commit you pushed** — it has no git awareness. **Run `git fetch && git status` and confirm `HEAD == origin/main` (nothing ahead or behind) before building**, and deploy from a clean tree rather than a feature worktree. On 2026-08-27 two parallel sessions each deployed their own tree; the second silently reverted the first's server code (`POST /v1/sesizari` 404'd in production) while git history stayed linear and conflict-free. The database will NOT reveal this — migrations are applied out-of-band, so the reverted feature's tables and RPCs remain and schema presence is not evidence the code is live. After deploying while others might be, smoke-test one route per recently-shipped feature (a missing route 404s where a live one 401s). See error-log #96.
...
- **Runbook: `docs/runbooks/monitoring.md`** — access paths, standard health-check queries, healthy baselines, known-benign error catalog. Sentry: one project `defensive-pedal-mobile` for BOTH mobile + API (discriminate by release prefix); MCP plugin for reads (interactive OAuth), `SENTRY_AUTH_TOKEN` in `apps/mobile/.env` (token "Defensive Pedal (build + monitoring)", created 2026-08-31: `alerts:read, event:read, org:read, org:write, project:read, project:releases, project:write`). ⚠️ Its predecessor lacked **`event:read`**, so every issue query 403'd with a body identical to an expired token — only the `www-authenticate: … insufficient_scope` header revealed it, and new-issue triage was blind the whole time (error-log #97). Sentry scopes are **immutable on an existing token**; fixing it means creating a new one. **Don't hand-roll an issue query: run `npm run monitor:sentry:new-issues`** (`scripts/sentry-new-issues.mjs`) — native path when `event:read` is present, Discover set-difference fallback (needs only `org:read`) otherwise, and it prints which mode it used. **Sentry's alert-rule APIs return HTTP 410**, so alert rules must be made in the UI. Sentry's burst rule 733370 emails on `defpedal-api*` error *spikes* — it cannot catch a 100%-broken **low-traffic** endpoint (~6 events/week never spikes; error-log #83). **GCP alerting (added 2026-08-21, the project's first — previously none existed):** two log-based policies, both per-occurrence, both emailing the *Victor (defpedal ops)* channel, rate-limited 1/h — *Cloud Scheduler job failed* (`10278737109769293908`, any `severity>=ERROR` in `cloudscheduler.googleapis.com/executions`, covers all 14 jobs) and *API 5xx response* (`4031446777758626143`). Channel **VERIFIED 2026-08-21** (delivery proven via `sendVerificationCode` → `:verify`). ⚠️ **`monitoring.googleapis.com` must stay enabled** — it was DISABLED when these policies were created, and creation still returned 200 with real IDs that read back `enabled: True`; only `sendVerificationCode` exposed it via `403 SERVICE_DISABLED`. A policy existing is NOT evidence that alerting works. **Sentry first-seen ISSUE rule added 2026-08-22** (UI-only, unverifiable from here) — it covers what GCP structurally cannot: swallowed exceptions and mobile crashes, neither of which returns a 5xx. `gcloud alpha/beta monitoring` cannot be installed here (needs admin on the SDK dir); use the Monitoring REST API with `gcloud auth print-access-token`. PostHog: EU project 162527, personal API key at `C:\dev\adminInfo\posthog\personal-api-key.txt` (NEVER commit), HogQL via `POST /api/projects/162527/query/`.
...
- ✅ **Uploads are HEADLESS since 2026-09-15** — `node scripts/play-publish.mjs --aab <path> --notes <dir>`, runbook `docs/runbooks/play-publishing.md`. Service account `play-publisher@gen-lang-client-0895796477.iam.gserviceaccount.com`, JSON key at `C:\dev\adminInfo\google_play_publisher\` (**outside the repo**, path overridable via `PLAY_SERVICE_ACCOUNT_KEY`). Play was the last manual step in the release pipeline. ⚠️ **Play permissions come from Play Console → Users and permissions, NOT from GCP IAM** — the service account holds no project roles, and a valid key with no Console grant authenticates fine then returns `403 PERMISSION_DENIED` on every call. That split is the diagnostic, not a broken key. `--status` defaults to `draft` (created, NOT rolled out) because a Play rollout has no undo.
- ⚠️ **The live store listing has exactly ONE locale: `en-GB`** (verified against the Play API 2026-09-16, replacing a long-standing "default listing language unverified" note). Not `ro-RO`, not `en-US` — so every country is served the English (UK) page, and **Play rejects release notes for any locale that has no listing**, which is why ro-RO/es-ES release notes never take effect. `apps/mobile/store-listing/{en-US,ro-RO,es-ES}/` holds title/short/full copy for three languages that have **never been applied**: the app is localized EN/RO/ES while its storefront is English-only. Adding the two listings is a product decision, not a fix.
...
## Hazard Imports (external civic-report ingestion)

Plan + full implementation record: **`docs/plans/hazard-import-pipeline.md`**.

- **Why:** production had **11 hazards, all expired** — the live map showed zero. This bootstraps the layer so the community flywheel has something to start from.
- **Source registry is the control surface.** `hazard_import_sources` (service-role only). Adding a city is an INSERT — but see error-log #98 before assuming it is *only* an INSERT: Zaragoza broke the "config-only" promise three ways (numeric ids, milliseconds rejected in dates, `page=` silently ignored in favour of a `start=` offset), none of which failed loudly. The runner **fails loudly** if a source is enabled without a registered adapter. Live sources (2026-09-01): `signalen:amsterdam` (2,729), `open311:koln` (182), `open311:zaragoza` (62), `civia` (47).
- **civia.ro consent GRANTED 2026-09-01 — for the PUBLIC pages only.** `Disallow: /api/` and the EU DSM Art. 4 TDM reservation still stand, so the adapter reads `feed.xml`, `sitemap.xml` and `/sesizari/<id>` and must never touch `/api/*` even though those endpoints exist and return cleaner JSON. Coordinates are parsed out of a **Next.js RSC flight payload** (`{"coords":[lat,lon],"zoom":16,"strada":null}`) — the one genuinely brittle parser in the pipeline; it fails loudly with the real HTTP reason when their build changes. Expect to fix it periodically. If Civia ever grants API access, **replace** the parsing rather than widening the scrape.
- **Sesizări round-trip suppression.** We *feed* civia.ro (riders hand off hazards to it), so an import can be our own rider's report coming home as a second pin, with our outbound volume masquerading as independent corroboration. `suppressRoundTrips()` in `imports/run.ts` drops an import when the `sesizari` ledger holds the **same hazard type** within **120 m** in the last **30 days**; ambiguous categories are always kept. Counted as `roundTrip`. RO-only.
- **`open311:zaragoza` (Spain, enabled 2026-09-01).** The only usable Spanish source found in a full sweep — Barcelona IRIS publishes annually, Madrid monthly, Málaga is stale since 2023 (`docs/plans/hazard-import-pipeline.md` §18). Zero latency, real open/closed status, ~30–50 requests/day. **Licence is UNVERIFIED** (portal legal page 404s; datos.gob.es reports null for publisher L01502973) — settle before public redistribution. Only ~46% of its requests carry coordinates; the rest are dropped as `badCoords`, which is correct and costs half the raw volume.
- **Cologne licence: Datenlizenz Deutschland – Zero – 2.0** (public-domain-equivalent, no attribution obligation). The `CC BY 4.0` on their site is **BKG basemap tile attribution, not the data** — don't re-derive the licence from the page. The impressum's "prior written permission" clause covers site content generally, not the released dataset.
- **`expires_at` had NO per-type TTL for the app's entire history** until 2026-08-27 (migration `202608270001`). The column DEFAULT `now() + 24h` pre-empted the `hazard_set_expiry` BEFORE-INSERT trigger, whose guard is `IF NEW.expires_at IS NULL` — Postgres applies column defaults *before* BEFORE-INSERT triggers, so `hazard_baseline_ttl()` was dead code and every hazard lived 24 h. **Never re-add a default to that column.** Verified live: `pothole` 24h → 336h, `narrow_street` → 720h.
- **`construction` is NOT a valid `hazard_type`** — removed from `hazards_hazard_type_check` by `202604210003` (swapped for `aggro_dogs`); inserting it returns 400. Roadworks map to `narrow_street`. `IMPORTABLE_HAZARD_TYPES` in `lib/imports/types.ts` is the guard, and every model verdict is validated against it.
- **Never publish imports through `POST /v1/hazards`.** Every side effect there is gated on `if (user?.id)` — `award_xp`, `qualifyStreakAsync`, `post_hazard_thanks` push, and `autoPublishHazardStandalone` to the social feed. Imports write the table directly with `user_id = NULL`, which suppresses all four. A service identity would spam the activity feed with hazards nobody reported.
- **Two Cologne API traps.** (a) `?service_request_id=a,b,c` on `/requests.json` is **accepted and silently ignored** — it returns the 100 most-recent rows instead, which made status-sync a no-op that still reported a healthy `statusChecked`. Use the per-id endpoint `GET /requests/{id}.json`. (b) `title` is literally `#<id> <service_name>`, so deriving a description from it yields German text leaking the city's ticket number. Every mapped category carries a reviewed English phrase in `mappings/koln.ts` instead.
- **`alert_eligible`** (on `hazards`, defaulting true) gates the navigation proximity-alert stack ONLY — map rendering ignores it. Sources with address-geocoded rather than reporter-pinned coordinates ship `false` so a bad geocode cannot fire mid-ride haptics. Cologne + Amsterdam are `pin` → true; civia and Zaragoza are `geocoded` → false.
  ⚠️ **It protects nobody today.** Its only client consumer is one line in `navigation.tsx`, first shipped in **v0.2.129** — and the fielded fleet is **v0.2.123** (Sentry release health, 2026-09-01). Every rider runs a build that ignores the field, so imported pins alert exactly like rider sightings, and the "Reported via …" provenance line is missing too. Setting it false is correct data and an inert safeguard until ≥0.2.129 ships. See error-log #100.
- **Cron:** `POST /v1/imports/run` (Bearer `CRON_SECRET`), weekly, cursor persisted **per page** (error-log #82 — a truncated cron re-grinds its prefix forever without one). Classification runs at concurrency 6 with an in-loop deadline check; items parked by the budget are retried by `reprocessStalled()` on the next run — **only** transient reasons, never `llm_low_confidence`/`llm_invalid_type`, which are genuine human-review states.
- **Dead-endpoint detection is TIME-based** (`last_items_at` / `stale_after_days`, default 8), not per-run. An empty run is normal: the cursor advances to `now` once a window is exhausted, so any back-to-back run legitimately fetches nothing. The fast failure modes (migrated endpoints returning an HTML shell, HTTP errors) already throw immediately in the adapter.
- **`OPENAI_API_KEY` is OPTIONAL in `validateConfig()`** — a missing/revoked key must degrade imports to the deterministic path, never `exit(1)` the rider-facing API. ⚠️ Local dev: a **machine-level (User scope) `OPENAI_API_KEY`** shadows `.env`, because `resolveConfigValue` checks `process.env` first. Likewise `CRON_SECRET` must be **exported** — `verifyCronAuth` reads `process.env` directly.
- **Amsterdam (`signalen:amsterdam`) — live since 2026-08-27, 2,051 hazards.** Adapter is `signalen`, named for the open-source platform (signalen.org) that several Dutch municipalities share, so a second Dutch city is a registry row, not new code. Three verified API constraints shape it: (a) **hard 4,000-feature cap with no paging** → the adapter quadtree-subdivides the city bbox, queueing child tiles in `cursor.tiles` so a truncated run resumes mid-sweep; (b) **every date filter is silently ignored** (`created_after`, `created_at_after`, `start_date` all return the same set back to 2021) → recency is filtered client-side by `isRecent()`; (c) **no id, no status, no free text** → the external id is synthesised as `lon:lat:created_at` (7dp rounding so a float-formatting change can't mint duplicates), expiry is TTL-only at 21 days, and **the model is never invoked** (a test fails if it is). Mapping uses a **parent allowlist** — an unknown slug is reviewed only under the three hazard-bearing parents, and drops under the other eleven; a blocklist would silently queue every new slug under a parent nobody excluded.
- ⚠️ **Amsterdam's licence is UNCONFIRMED — enabled by explicit product-owner override on 2026-08-27.** The feed returns zero hits on data.overheid.nl, so "publicly reachable at a `/public/` endpoint" is the only basis; that is not a licence grant. `hazard_import_sources.licence` records this as `UNCONFIRMED-OWNER-OVERRIDE-2026-08-27` rather than a real licence, deliberately, so nobody later mistakes it for settled. If the city objects, rollback is `UPDATE hazard_import_sources SET enabled=false WHERE id='signalen:amsterdam';` then `DELETE FROM hazards WHERE import_source='signalen:amsterdam';`. Cologne is unaffected — its DL-DE-Zero-2.0 grant is confirmed and documented above.
...
- **`/course-import` takes EITHER a file `uri` or a saved `courseId`** — one screen for both paths, and the same single-input shape the OS "Open with" intent uses.
- **"Open with Defensive Pedal" is live** (2026-09-04). Riders receive courses as mail/Drive attachments, so this matters more than the in-app picker. **Android filters are hand-written in `AndroidManifest.xml`** — `intentFilters` in `app.config.ts` would NOT land, because this project never runs `expo prebuild` (error-log #27). **iOS is the opposite**: `CFBundleDocumentTypes` + `UTImportedTypeDeclarations` in `app.config.ts` DO apply, because iOS builds go through EAS, which prebuilds on the server. GPX is not a system UTI — without the imported-type declaration, `LSItemContentTypes` names an identifier iOS has never heard of and the app never appears in the share sheet.
- ⚠️ **Never widen the Android filter to `android:mimeType="*/*"`.** It would offer this app as a handler for every file on the device. Two filters instead: correct MIME types (`application/gpx+xml`, `application/xml`, `text/xml`) with no path constraint, because cloud content URIs carry no extension to match on; and `application/octet-stream` (what Drive reports) NARROWED by a `.gpx` `pathPattern`.
- **`GpxOpenHandler` (`app/_layout.tsx`) stages the file before doing anything else.** Android’s read grant on a `content://` URI lasts only as long as the receiving task, so the file is copied into our cache immediately and the borrowed handle is dropped; navigation is then suppressed during `NAVIGATING` and during onboarding (a fresh install opening a .gpx must not be yanked past the mandatory signup wall), with the staged path held in the **transient, non-persisted** `pendingCourseImport` slot. Persisting it would re-navigate on every cold start at a cache path that may be gone.
- **Not shipped:** ACTION_SEND ("share to Defensive Pedal"). Android puts that URI in `EXTRA_STREAM`, not the intent data, so `Linking.getInitialURL()` returns null for it — supporting it needs native code or `expo-share-intent`, not another manifest entry. Also unshipped: risk-segment proximity alerts during a ride, and opt-in map-matched street names.

## Risk Display Bands — b46v1 re-anchoring (2026-09-04)

Rationale + the accident validation behind it: **`BAND_REANCHOR_B46V1.md` in the
OSRM_Server repo** (`C:\dev\OSRM_Server`). Runbook: `docs/runbooks/road-risk-data.md`.

- **Three claim tiers over ten display shades.** `Safer` / `Typical` / `High risk`,
  plus `No data`. The b46v1 generation (live 2026-09-01) scores ~13 points higher
  and wider than the previous cuts assumed, and the validation supports exactly
  three *nameable* levels. Hue changes only at a tier boundary; lightness varies
  within a tier — so the map still shows gradation without us implying we can tell
  two shades of one tier apart.
- ⚠️ **THE claim rule: the tier is the only level at which risk may be named or
  compared.** Never label, rank, or write copy about an individual shade — in UI,
  i18n, a chart axis or a commit message. This has already been violated once: the
  shipped `typical` band description said "darker shades lean busier" in all three
  locales and had to be removed (session 114).
- ⚠️ **The one validated claim is about SEVERITY, not frequency.** A crash on a
  **High risk** road is more likely to be *serious*. Do NOT write copy implying a
  high-risk road is where crashes are more *likely* — that is a different claim
  and it is not validated.
- **Score cuts are server-side only** (2026-04-13 risk-IP hardening). They live in
  `services/mobile-api/src/lib/risk.ts` — `RISK_BUCKETS`, which also owns the
  per-shade colours, the tier label sent as `riskCategory`, and the quantized
  midpoints. Never mirror a threshold into the client or a doc. **Colours ARE fine
  to bundle** — they are painted on the map in front of the rider anyway; the cuts
  are the IP, not the palette.
- **Client mirrors of the palette, and their guards.**
  `apps/mobile/src/design-system/tokens/riskLegend.ts` carries the tier → shades
  map; `packages/core/src/riskDistribution.ts` `RISK_CATEGORY_ORDER` carries the
  four labels; `packages/core/src/riskStretch.ts` `BUSY_ROAD_CATEGORIES` is
  `{'High risk'}` only, and is the single definition of "busy" shared by the
  route-preview callout and the imported-course busy-stretch list (error-log #20).
  `riskLegend.test.ts` asserts the shades match the server list, that all ten are
  present and unique, and that **no numeric field** exists on a band — i.e. that
  nobody reintroduced cuts client-side.
- **Legends render a RAMP, not a swatch.** Both `RiskScoreExplainerSheet` and
  `RiskDistributionCard` show each tier's shades. In the distribution card this
  fixed a real bug, not just a cosmetic one: `computeRiskDistribution` keeps the
  FIRST colour it sees per category, so a single dot took whichever shade happened
  to appear first on that route and the same tier drew differently from route to
  route. An unrecognised label still falls back to the flat server colour — never
  guess a band. `No data` is deliberately one flat blue: it is the absence of a
  rating, not a level.
- **Old builds degrade gracefully by design.** They paint the server-sent colours
  and render the raw English tier label through `riskBandKeyForServerLabel`
  returning null for a label it does not know. The fielded fleet was v0.2.123 when
  this shipped, so most riders saw exactly that. Do not "fix" the unknown-label
  fallback into a guess.
- **Retired vocabulary** (`Very safe` / `Safe` / `Average` / `Elevated` / `Risky` /
  `Very risky` / `Extreme`) must not reappear. Note `apps/web` uses a SEPARATE
  0-100 **safety** score (higher = better) whose words were deliberately made
  non-colliding — do not "align" it to the tier names, it is a different scale.
  The `mia:` i18n block and `docs/prd-mia-persona-journey.md` still contain
...
│   │   ├── faq.tsx              # FAQ shell — content lives in src/content/faq.{en,ro,es}.ts (4 sections, 25 Q&A items ×3 locales)
...
│   │   │   └── ProfileDeviceSyncManager.tsx   # Syncs device timezone + app locale to profiles at session bootstrap
│   │   └── store/
...
- `OfflineMutationSyncManager` drains queue every 15s when API reachable, **plus immediately (300ms debounce) when a trip-critical mutation is enqueued** (GPS audit 2026-07-29: ride end is when riders kill the app, and waiting for the 15s tick lost fully-ridden trips on devices that never reopened — the impact POST landed, the queued trip_end didn't). The kick is a store subscription with a `queuedMutations` reference-equality fast path; don't remove it, and don't make it fire on status changes (new mutation IDs only). Trip-critical deliveries and dead-letters also fire-and-forget `telemetry.flush()` so the evidence outlives an immediate app-kill
...
- `trips` → lifecycle metadata only; not read by any user-facing screen. Since 2026-07-29 it carries `end_action` (`saved`/`discarded`/`prompt_saved`/`prompt_discarded`/`auto_recovered` stamped by v0.2.122+ clients; `abandoned` stamped ONLY by the nightly stale-trip reaper on the hazards-expire cron — in_progress >48h, `ended_at` deliberately stays NULL; a late genuine trip_end overwrites the stamp). Loss triage query lives in `docs/runbooks/monitoring.md` health check #7 — never "fix" a stuck trip by faking `ended_at`
...
**4b. Push LANGUAGE + TIMEZONE come from `profiles`, synced by the client (2026-08-13).** `profiles.preferred_locale` ('en'|'ro'|'es', nullable) drives every server-sent push; resolve it through the single permissive helper `toNudgeLocale` (`lib/nudges/locale.ts` — accepts region tags like `es-ES`, falls back to `en`; `normalizeFirstRideLocale` delegates to it). Before this column existed every dispatch hardcoded `'en'` and the commissioned RO/ES pedalVoice catalogs were dead code. `profiles.quiet_hours_timezone` is nullable with NO schema default (the old `'Europe/Bucharest'` default meant quiet hours ran on the Bucharest clock for anyone who never opened the Profile screen — Reykjavík riders were pushable from 04:00 local); ALL server fallbacks are `?? 'UTC'` — keep the nudge stack, `eventFirer`, and `lib/notifications.ts` in agreement. Both fields are pushed by `ProfileDeviceSyncManager` (mounted in `AppProviders`) at every session bootstrap, which also self-heals travelers and rows still carrying the old Bucharest stamp. Don't move that sync back into the Profile screen — that's exactly how the defaults went stale fleet-wide.
...
**8. Dead-token pruning has TWO halves — both must stay wired.** (a) *Immediate*: the send response's in-ticket `DeviceNotRegistered` is pruned inline by `notifications.ts` / `nudges/dispatcher.ts`. (b) *Delayed*: a token can be ACCEPTED and then fail at the FCM/APNs hop, which Expo reports only in the **receipt**, minutes later — structurally invisible to (a). That half runs on `push-receipts-cron` (`*/30 * * * *`) → `POST /v1/notifications/receipts/check` → `lib/pushReceipts.ts`, backed by the `push_receipts` table (migration `202608180001`), which is the ticket_id→token map a receipt needs (a receipt carries no token, and neither `notification_log` nor `nudge_log` stores one). Every accepted ticket is recorded there at send time; rows delete as receipts resolve and age out at Expo's ~24 h retention, so the table stays ~one send-batch in size. Wired 2026-08-18 (audit SCALE-18); before that `checkReceipts` had shipped for two months with zero callers. **Why it matters:** Expo deprioritises senders with high error rates, so unpruned dead tokens degrade delivery for every user — the reputational mechanism behind error-log #69. If you add a new send path, record its accepted tickets via `recordPushTicket` or its dead tokens are never reaped.
...
| **Auth email links = direct token_hash to edge fn (no /auth/v1/verify)** | Since 2026-08-11 the Confirm-signup + Recovery email templates link straight at the `email-confirm` edge function with `{{ .RedirectTo }}&token_hash={{ .TokenHash }}&type=…`. The old `{{ .ConfirmationURL }}` routed through single-use `GET /auth/v1/verify`, which mail-scanner prefetch / double-taps / stale re-signup emails consumed before the user, and Chrome blocks `intent://` from omnibox-pasted navigations — the "confirmation email throws an error" class of bugs. With token_hash links, browser GETs consume NOTHING; only the app's `verifyOtp` does (no PKCE verifier needed → cross-device + reinstall-safe). Templates live in Supabase auth config (Management API `PATCH /config/auth`), NOT the repo — never revert them to `{{ .ConfirmationURL }}`. Edge fn deploys REQUIRE `--no-verify-jwt` (no config.toml pins it). Desktop visitors land on `routes.defensivepedal.com` pages (`/email-confirmed`, `/email-open-on-phone`, `/email-link-expired`); the web app deploys from the REPO ROOT (Root Directory = `apps/web`). "Resend confirmation email" affordance on auth.tsx. Full flow: `supabase/functions/email-confirm/README.md`, error-log #77 |
| **Expo Modules API guard before `require('expo-notifications')`** | `require()` of a missing native module causes uncatchable fatal crash on Android. Detect presence first via `hasNotificationsNativeModule()` (`apps/mobile/src/lib/notificationNativeModule.ts`), which probes `requireOptionalNativeModule('ExpoPushTokenManager')` from `expo-modules-core`. **Do NOT check `NativeModules.ExpoPushTokenManager`** — it's `undefined` on the New Architecture (bridgeless) preview/production builds even when the module is present, silently killing all notifications. See error-log #21b + #2b |
...
| **Cool mode is LIVE in production since 2026-09-18** (dev/preview-only from 2026-08-29 until then) | A product decision both times, not a defect. `isCoolModeEnabled()` in `apps/mobile/src/lib/coolMode.ts` now returns `true` unconditionally and is kept as the kill switch — **to hide Cool again, return `false` there; nothing else needs undoing**, since the routing dispatch, the `HEAT_ROUTING_COUNTRIES` coverage gate, the shade OSRM instance and the `saved_routes.avoid_heat` column are all untouched. ⚠️ The gate is on the **store setter and rehydration**, not just the pill: `avoidHeat` has FIVE entry paths — the Cool pill, route-preview's tap-to-cycle pill, claiming a shared `'cool'` route, opening a saved route with `avoid_heat=true`, and persisted state from an earlier session. All five coerce through `resolveAvoidHeat`, which is what makes flipping the flag one line rather than an audit; path 3 is not hypothetical — it reached production once, serving shade-graph routes while the mode pill read "Safe", because `setRouteRequest` was the one setter that did not coerce. ⚠️ **The remaining gates on Cool are real, not build flags:** `isHeatRoutingAvailable(country)` for shade coverage, and `resolveCoolRoutingAvailability` for entitlement (`PLUS_MODES_FREE_UNTIL` — free to everyone through 2026-09-30, Plus after). The paywall's cool benefit (`profile.tsx` `coolRoutingAvailable`) is deliberately a COUNTRY check only, which is correct now that the build flag is always true. |
| **Cool routes in all 31 covered countries (2026-09-17)** — was RO-only | `HEAT_ROUTING_COUNTRIES` (core `countryCoverage.ts`) now equals `ROUTING_COVERED_COUNTRIES`, so the Cool pill, the preview cycle-pill, both dispatchers (app `mapbox-routing.ts`, API `customOsrm.ts`) and the paywall's cool benefit all widened at once. MEASURED before widening: `osrm-shade` returned a real route from the capital of all 31 countries. ⚠️ **"Routes" is not "cools".** The heat model's canopy data covers 119 city points in 26 countries (`eu_uhi_cities.json` in OSRM_Server); none resolve to LU/SI/LI/CH (PT sits inside the ES bbox), so outside those cities a Cool route is just the shade graph's standard routing. ⚠️ And the shade graph is a generation behind Safe (b36 vs b46, `HANDOFF_b46v1.md` item 1: rebuild decided, not done) — Cool differed from Safe in 25/31 capitals, including LU and CH where no heat data exists, so some of that difference is model age, not shade. Still hidden in production by `isCoolModeEnabled()` — this widened coverage, not visibility. |
...
| **Routed ETA = router duration** (no app climb penalty on top) | OSRM (all four graphs) and Mapbox Directions cycling already slow the rider on climbs inside their own duration. MEASURED 2026-09-16 by riding the same road both ways at identical distance against the live services: uphill is slower by MORE than the app penalty's own up/down gap on every climbing pair in 6 countries (OSRM Sinaia→Cota 1400 2,072 s vs 787 s, Tibidabo 489 vs 191; Mapbox Brașov→Poiana 914 vs 336, Stuttgart 410 vs 147), flat controls symmetric (Copenhagen 1.00 on both). The Lua reason for OSRM: `grade_speed_factor` scales way speeds (bicycle46/-flat/36), and §E1 power-balance climb speed in bicycle46-ebike. So the old 0.75 s/m + 10 s/climb penalty counted every climb twice for the app's whole life (Râșnov→Poiana previewed 1 h 41 for OSRM 1 h 30), and navigation — which sums raw step durations — contradicted its own preview. `enrichRouteWithElevation` now REQUIRES `durationIncludesClimbs`: `true` for route preview/reroute and loops, `false` only for GPX courses, whose duration is distance at a constant 15 km/h and genuinely knows nothing about climbs. Mirrored server-side in `normalize.ts` and `loops/measure.ts` (parity test updated). **Live 2026-09-16:** main `d6470d2`, Cloud Run `defpedal-api-00168-gzc` (digest-verified against the build of that commit), and **device-confirmed on preview v0.2.165**, Râșnov → Poiana Brașov: Safe 1 h 41 → 1 h 30, Fast 1 h 05 → 55 min, Flat 1 h 40 → 1 h 30, E-bike 1 h 16 → 1 h 04, Cool 1 h 53 → 1 h 31 — each exactly the raw router duration, distances unchanged. Error-log #124 |
...
8. **`expo-notifications` native module crash** — guard with `hasNotificationsNativeModule()` (Expo Modules API probe) before `require()`. NEVER use `NativeModules.ExpoPushTokenManager` — it's `undefined` on bridgeless preview/production builds and silently disables all notifications (error-log #21b + #2b)
...
11. **Auth email links must NOT route through `/auth/v1/verify`** — that GET is single-use: mail-scanner prefetch (Outlook SafeLinks/AV), double-taps, and stale re-signup emails consume it before the user ("Email link is invalid or has expired"), and Chrome blocks the follow-up `intent://` hop on omnibox-pasted links. Confirmation + recovery templates link directly at the `email-confirm` edge fn with `{{ .RedirectTo }}&token_hash={{ .TokenHash }}&type=…`; only the app's `verifyOtp` consumes the token (error-log #77, `supabase/functions/email-confirm/README.md`)
...
> Console. ⚠️ **It went out at 100% immediately, NOT staged** — the Play API
> reports the production release as `status: completed` with no `userFraction`,
> so the 5%→20%→50%→100% cadence with the vitals gate below was not used this
> time, and release notes landed in **`en-GB` only** (the sole listing locale —
> see § Play Store Release). iOS: **1.19 / build 30** `WAITING_FOR_REVIEW` with
...
- Cycling weather notification with randomized cadence (since 2026-07-18; previously daily 8:30am) — random intervals 12h–120h (2x/day … once per 5 days) persisted in `dailyWeatherChain`, plus daily escalation fires for users inactive ≥3 days; all fires snap into the 08:30–21:00 waking window (cadence math in `daily-weather-schedule.ts`, 29 tests). Content: 40 witty/sarcastic random title variants on good-weather days (temp 10–28°C, rain ≤30%, wind ≤25 km/h, no storm/snow) — **drawn without repeats since 2026-08-12**: each scheduling pass excludes titles already assigned this pass plus the previous pass's persisted set (`dailyWeatherRecentTitles` in Zustand, device-scoped) so consecutive deliveries never reuse a line; safety warnings on bad-weather days (storm / snow / extreme cold / strong wind / heavy or moderate rain / freezing / windy) deliberately repeat verbatim and never enter the memory. Pure helpers (titles, advice builder, forecast parser) live in `daily-weather-messages.ts`. SUT fetches an 8-day forecast BEFORE cancelling the prior schedule so a fetch failure leaves the queued set intact
...
- **Habit Engine:**
  ...
  - 3-screen onboarding flow (intro/location → region check → signup prompt). History: 2026-07-04 (`592b751`) cut safety-score/goal-selection/first-route (left orphaned, **files DELETED 2026-08-13** along with ~45 dead `onboarding.*` keys ×3 locales — session-103 work had accidentally landed on the dead first-route, and `safety-score.tsx` rendered a FABRICATED fallback score as if real); 2026-07-16 re-inserted the signup prompt as the final step, then the consent screen was removed the same day (`consent.tsx` DELETED — telemetry moved to a first-screen transparency notice + Profile › Privacy & analytics). `region-check` now owns marking onboarding complete (before the prompt, so a hard-close doesn't re-run the flow) + the once-only anonymous open-count reset. Signup prompt also still fires as the anonymous re-prompt (2nd cold open dismissible, 3rd+ mandatory via `computeOnboardingGateTarget`)
  ...
  - Daily safety quiz — three country-dispatched static pools (RO 44 / ES 44 / **GENERIC 46**, all trilingual), streak qualifier. `resolveQuizCountry` (core) picks: Profile override → GPS bbox → device-locale region → **GENERIC default**, and the API defaults to GENERIC too (was `'RO'` until 2026-08-13 — an omitted param served Romanian road law worldwide). GENERIC content must stay country-agnostic (no traffic code / fines / authority names) and the pool must stay LARGER than the 30-day answer cooldown — at 30 questions it emptied and 404'd, which the client renders as a permanent "Failed to load quiz"; exhaustion now falls back to the least-recently-answered question, and a test floors the pool at 40. See error-log + `docs/reviews/geo-remnants-review-2026-08-13.md` G-02/G-19/G-20.
...
- **Help & FAQ**: 25 Q&A items in 4 sections (Safety & Routing, Your Impact, Progression & Rewards, Privacy & Data), fully localized — content modules `src/content/faq.{en,ro,es}.ts` selected by app locale (restructured 2026-08-13, review G-10; ids stable across locales). Accessible from Settings tile, Profile > Account row, and History tab card
- **Stability & UX:**
  ...
  - Stale auth token recovery: supabase-js reports an expired/invalid refresh token as a null session (no throw) and the app falls through to anonymous sign-in. A THROW from the session read is a storage failure, not an expired token, and since 2026-09-10 is kept and retried rather than cleared (`lib/sessionReadError.ts`; error-log #116)
...
- ~~**Play Console default listing language unverified**~~ — **RESOLVED 2026-09-16** by reading the Play API directly: the live listing has exactly one locale, **`en-GB`**. The feared "ro-RO is default, ~28 countries get the Romanian page" scenario is not what happened — everyone gets the English (UK) page. Still open as a *product* choice: `apps/mobile/store-listing/{en-US,ro-RO,es-ES}/` has never been applied, so a RO/ES-localized app has an English-only storefront. See § Play Store Release.
...
- **Units: metric or imperial, per rider (2026-09-23).** `measurementSystem` in the store (device-scoped, persisted, seeded from the region gate's country — GB → imperial — by `setRegionGate` and by persist migration v8; Profile › Display toggles it). Distance/climb/speed render ONLY through the core formatters, which take `units` as a REQUIRED argument so a new call site cannot silently ignore it — never add a default. Rules + conversions live in `packages/core/src/units.ts`; `spokenDistance()` (mobile) is the spelled-out form for speech and screen readers. i18n strings must NOT contain a unit — pass an already-formatted value. Temperature, weight and CO2 stay metric on purpose.
...
- `profiles` push-relevant columns: `preferred_locale` (nullable 'en'|'ro'|'es' — server push language) and `quiet_hours_timezone` (nullable, **no default**; server falls back to UTC). Both synced from the device by `ProfileDeviceSyncManager` — see Notifications §4b. Migration `202608130001`.
...
- **One graph covers all 32 supported countries** (EU-27 + EEA + CH + UK — see `packages/core/src/appAvailability.ts`); verified 2026-07-12 by probing Berlin/Paris/Madrid/Stockholm/Reykjavik/Nicosia + a Vienna→Bratislava cross-border route. **UK since 2026-09-21 (b47v1, all four arms incl. shade + e-bike; UK risk data swapped the same day):** probed from the app side 2026-09-22 — London/Belfast/Edinburgh/Lerwick/Stornoway/Penzance/Scilly route, Dublin→Belfast crosses the land border, London→Paris is `NoRoute` (no Channel link in the graph), Isle of Man + Channel Islands are NOT in the graph (distance-0 → Mapbox via the guard). The former `osrm-es.*` / `osrm-es-flat.*` per-country pair is retired (config keys + dispatch removed).
- **Out-of-coverage failure shape**: OSRM does NOT error for points outside its data (Serbia, Canaries, Isle of Man…) — it returns `Ok` with a degenerate distance-0 route. Both the client (`fetchOsrmRoutes` → `OsrmOutOfCoverageError` → silent degrade to Mapbox fast + coverage `unsupported`) and the server (`customOsrm.ts`) carry a **zero-distance guard**; do not remove it — the loose coverage bboxes deliberately over-include neighbors (Bosnia inside the HR box, Belgrade/Chișinău inside the RO box) and rely on the guard as the backstop.
...
- **Coverage dispatch**: `isRouteSupported` in `packages/core/src/countryCoverage.ts` — 32 per-country bbox sets (`COUNTRY_BBOXES`), **cross-border pairs within coverage are SUPPORTED** (`cross_border` reason removed). RO + ES stay listed FIRST (first-match attribution; ordering was load-bearing while risk data was RO/ES-only, cosmetic since — locked by a test either way); **GB is listed third, BEFORE FR and IE, and that order IS load-bearing for attribution** — the loose FR box reaches 51.2°N and swallows the English south coast (Dover → Plymouth), and the GB boxes are cut to avoid French and Republic-of-Ireland land so GB-first steals neither (tested landmark-by-landmark in `countryCoverage.test.ts`); most of Northern Ireland still attributes IE (cosmetic); Iberia note — Portugal attributes as 'ES' (inseparable by bbox, harmless). Canary Islands / Madeira / Azores / Samos remain outside the boxes (no graph data → Mapbox fallback). **`road_risk_data` is EU-WIDE since 2026-08-01** (b36v1 generation, 67.9M segments, generated alongside the routing graph via `export_risk_eu.py` in the OSRM_Server project — "risk data matches routing"): risk overlays, Risk Score card, and the safe-vs-fast comparison work in every covered country (UK included since b47v1), gated by `RISK_DATA_COUNTRIES`/`isRiskDataAvailable` in core (= full routing footprint; narrow that ONE list if data/routing generations ever diverge again). The May-2026 v22 RO+ES state (6.15M rows) described below is historical.
