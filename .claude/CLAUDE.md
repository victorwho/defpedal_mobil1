...
- **Source registry is the control surface.** `hazard_import_sources` (service-role only). Adding a city is an INSERT, not a deploy. Live: `open311:koln` (enabled) and `civia` (**disabled pending consent** — civia.ro blocks every AI-crawler UA, disallows `/api/`, and files an EU DSM Art. 4 TDM reservation; its adapter is deliberately not written. The runner **fails loudly** if a source is enabled without a registered adapter).
...
- **`alert_eligible`** (on `hazards`, defaulting true) gates the navigation proximity-alert stack ONLY — map rendering ignores it. Sources with address-geocoded rather than reporter-pinned coordinates ship `false` so a bad geocode cannot fire mid-ride haptics. Cologne is `pin` → true; civia is `geocoded` → false.
