...
- **Coverage dispatch**: `isRouteSupported` in `packages/core/src/countryCoverage.ts` — 31 per-country bbox sets (`COUNTRY_BBOXES`), **cross-border pairs within coverage are SUPPORTED** (`cross_border` reason removed). RO + ES stay listed FIRST (first-match attribution; ordering was load-bearing while risk data was RO/ES-only, cosmetic since — locked by a test either way); Iberia note — Portugal attributes as 'ES' (inseparable by bbox, harmless). Canary Islands / Madeira / Azores / Samos remain outside the boxes (no graph data → Mapbox fallback). **`road_risk_data` is EU-WIDE since 2026-08-01** (b36v1 generation, 67.9M segments, generated alongside the routing graph via `export_risk_eu.py` in the OSRM_Server project — "risk data matches routing"): risk overlays, Risk Score card, and the safe-vs-fast comparison work in all 31 covered countries, gated by `RISK_DATA_COUNTRIES`/`isRiskDataAvailable` in core (= full routing footprint; narrow that ONE list if data/routing generations ever diverge again). The May-2026 v22 RO+ES state (6.15M rows) described below is historical.
