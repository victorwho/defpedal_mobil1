...
| **Cool mode is HIDDEN in production (2026-08-29)** | Product decision, not a defect — dev/preview builds keep it. Gated by `isCoolModeEnabled()` in `apps/mobile/src/lib/coolMode.ts` (checks BOTH `appVariant` and `appEnv`, so a mis-set env fails safe). ⚠️ The gate is on the **store setter and rehydration**, not just the pill: `avoidHeat` has FIVE entry paths — the Cool pill, route-preview's tap-to-cycle pill, claiming a shared `'cool'` route, opening a saved route with `avoid_heat=true`, and persisted state from an earlier session. Hiding only the pill would strand anyone who already had it on in an invisible mode with no control to leave it, so `setAvoidHeat` refuses `true` and `onRehydrateStorage` coerces a stale `true` back to false. To re-enable: make `isCoolModeEnabled()` return true unconditionally — nothing else needs undoing, since the routing dispatch, `HEAT_ROUTING_COUNTRIES` coverage gate, the shade OSRM instance and the `saved_routes.avoid_heat` column are all untouched. |
