...
- **GitHub Actions CI on `main` has failed on EVERY push since at least 2026-09-04** (25 runs back; the only green run is a Dependabot docker job). Step `Security audit` (`npm run audit:ci`) reports 20 non-allowlisted high/critical advisories in production-classified deps: `next` (2 CRITICAL RCEs, apps/web), `@xmldom/xmldom` ×12, `fast-uri` ×4, `js-yaml`, `sharp`. Not caused by the R8/Sentry work (no dependency changed); the "all jobs failed" email on 2026-09-10 is this standing state. Needs real bumps (next first), not allowlisting — see `scripts/audit-ci.mjs` policy.
- **`AuthSessionProvider` signs out on ANY failure of the secure-store session read**, not only an expired refresh token (bare `catch {}` -> `signOut({ scope: 'local' })`). R8 exposed it (error-log #116); a transient keystore error on any build would do the same. Separate fix, not started.
