...
17. **Local CI parity?** Pre-push hook only runs typecheck + lint, NOT `npm audit`. CI's Security audit step runs `npm audit --omit=dev --audit-level=high` — it audits only PRODUCTION (shipped) deps, so a high/critical there fails CI, but dev-only tooling advisories (vitest/esbuild/postcss dev-server CVEs) do NOT. (Before 2026-06-09 it ran without `--omit=dev` and failed on every push once the dev-only vitest GHSA-5xrq-8626-4rwp critical landed.) Run `npm audit --omit=dev --audit-level=high` locally to mirror CI; run plain `npm audit` to also review dev-tool advisories. Fix shipped-dep advisories with `npm audit fix` (non-breaking) and re-pin where needed
