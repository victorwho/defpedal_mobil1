...
**Residue / rules:** (a) if pushes are ever wanted on dev-variant builds, the `.dev` package needs the same key uploaded (only `preview` exists as an Android application identifier in Expo credentials today — production installs get pushes via the same preview-package credential? NO: production tokens worked before this fix, so their credential path predates this; do NOT delete the preview key thinking it's redundant). (b) `eas credentials` cannot run inside Claude Code shells (no TTY — "stdin is not readable"); use the expo.dev web UI. (c) Improvement TODO: dispatcher could log per-token ticket errors into `nudge_log.context` so a partially-failing fan-out is visible.
