-- City Heartbeat: only show routes-planned for a window it actually covers
-- (2026-09-14)
--
-- Recording started this morning against an empty table, so the first render
-- put "1 Routes planned" beside "1422 Rides" on the COMMUNITY ALL-TIME card.
-- Both numbers are true and the comparison is meaningless: the ride count runs
-- from 2025-12-17, the plan count from 09:39 today.
--
-- Fix is structural, not a threshold. `routesPlanned` gains `coversWindow`,
-- true only when recording predates the window being shown, and the client
-- renders the cell only then — so it disappears now and returns by itself once
-- a full month has been recorded, with no follow-up release and no hardcoded
-- date. The all-time variants are dropped from the UI permanently; see the note
-- on them below.
--
-- Replaces get_city_heartbeat from 202609140003; every other block is byte
-- identical.
