-- City Heartbeat: surface municipal cycle counts (2026-09-14)
--
-- Adds `cyclingInEurope` from get_bike_counter_totals (migration
-- 202609140008). Paris measured 1,929,105 cyclists across 108 counters in seven
-- days — the only genuinely large "people cycling" number available, and a real
-- count of real people rather than anything derived from our own usage.
--
-- ⚠️ Labelled as Europe in the UI, never as local activity, and never summed
-- with rides. Romania publishes no cyclist counters at all and nearly all our
-- riders are Romanian, so "near you" would be both wrong and empty.
--
-- Replaces get_city_heartbeat from 202609140007; every other block is byte
-- identical.
