-- City Heartbeat: network-scale stats (2026-09-14)
--
-- The community numbers are honestly small — 822 signed-up cyclists, 552 who
-- have ridden — and no amount of framing makes them big without lying. What IS
-- big, and true, is the network those riders are riding on: ~67 million road
-- segments scored for safety and ~5,000 hazards mapped across four cities.
--
-- Adds a `network` block. Global rather than scope-resolved on purpose: it
-- answers "how much does this app know", which does not vary by where you are
-- standing. See the notes inside on why one is an estimate and the other is not,
-- and on what the hazard count may NOT be called.
--
-- Replaces get_city_heartbeat from 202609140006; every other block is byte
-- identical.
