-- Fix: check_and_award_badges should count completed trips from the trips
-- table, not just ride_impacts. The ride_impacts row is only created when
-- the impact endpoint is called, but trips are recorded immediately via
-- the offline queue. This means first_ride (and ride count badges) were
-- never triggered because ride_impacts was empty.
...
BEGIN
  ...
  INTO
    v_total_co2, v_total_money, v_total_hazards, v_total_riders_prot
  FROM profiles
  WHERE id = p_user_id;
  ...
  -- 3. Ride aggregates (from ride_impacts)
  SELECT
    COUNT(*)::INT,
  ...
  INTO
    v_ride_count, v_total_distance_m, v_total_duration_min,
    v_total_elevation_m, v_max_single_dist_m, v_max_single_elev_m
  FROM ride_impacts
  WHERE user_id = p_user_id;

  -- 3b. Also count completed trips (trips table is populated before ride_impacts)
  SELECT COUNT(*)::INT INTO v_trip_count
  FROM trips
  WHERE user_id = p_user_id AND ended_at IS NOT NULL;

  -- Use the higher of the two counts for ride-based badges
  ...
  grouped AS (
    SELECT d, perfect, d - (ROW_NUMBER() OVER (ORDER BY d) * INTERVAL '1 day') AS grp
