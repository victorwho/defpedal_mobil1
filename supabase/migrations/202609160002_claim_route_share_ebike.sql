-- claim_route_share: carry an 'ebike' share's pedelec preference onto the
-- claimed saved route.
--
-- Companion to 202609160001_saved_routes_is_ebike.sql. ORDER-DEPENDENT: this
-- RPC lists saved_routes columns explicitly, so applying it before is_ebike
-- exists breaks EVERY claim, not just e-bike ones. Apply 202609160001 first.
--
-- Before: routingMode 'ebike' fell through the CASE to 'safe' and set no
-- profile flag, so a claimed e-bike share silently became a plain safe route
-- (the mode CASE is unchanged — e-bike is a refinement of 'safe', exactly like
-- 'flat' and 'cool').
--
-- Body below is the LIVE definition read via pg_get_functiondef on 2026-09-16
-- (per the RPC-drift rule). It was verified identical to
-- 202608210002_claim_route_share_avoid_heat.sql before patching; only the
-- saved_routes INSERT column list and its last VALUES entry changed.
