-- City Heartbeat: estimated daily cyclists in the rider's city (2026-09-14)
--
-- Adds `cityCyclingEstimate` from get_city_cycling_estimate (migration
-- 202609140010). Bucharest resolves to ~73,000 and Brașov to ~10,000 — the
-- largest city-local "people cycling" figures available, and the only ones
-- possible at all given Romania publishes no counter data.
--
-- ⚠️ An ESTIMATE, and the UI must say so and show its source. The weak input is
-- a 3.4% cycling modal share measured across Romanian county seats in
-- 2015-2017, which is neither city-specific nor recent; it is more likely low
-- than high, which is the right direction for a headline number.
--
-- Replaces get_city_heartbeat from 202609140009; every other block is byte
-- identical.
