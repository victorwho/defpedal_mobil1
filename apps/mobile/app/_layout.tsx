 ...
 */
/**
 * One-time notice that Cool AND E-bike routing are free to everyone until
 * the promotion ends, and part of Plus after it.
 *
 * Three gates, all here rather than in the component:
 *  - shown once per device (`hasSeenPlusModesNotice`);
 ...
 */
const PlusModesPromoNoticeManager = () => {
  const hasSeen = useAppStore((s) => s.hasSeenPlusModesNotice);
  const setHasSeen = useAppStore((s) => s.setHasSeenPlusModesNotice);
  const appState = useAppStore((s) => s.appState);
  const onboardingCompleted = useAppStore((s) => s.onboardingCompleted);
