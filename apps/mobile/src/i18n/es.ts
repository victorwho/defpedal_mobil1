import type { TranslationKeys } from './en';

export const es: TranslationKeys = {
  // ── Common ──
  common: {
    appName: 'Defensive Pedal',
    cancel: 'Cancelar',
    save: 'Guardar',
    done: 'Listo',
    back: 'Atrás',
    close: 'Cerrar',
    loading: 'Cargando...',
    error: 'Error',
    retry: 'Reintentar',
    routeDetailsSheet: 'Detalles de la ruta',
    guest: 'Invitado',
    signedIn: 'Sesión iniciada',
    yes: 'Sí',
    no: 'No',
    km: 'km',
    unitMeters: 'metros',
    unitKilometers: 'kilómetros',
    unitFeet: 'pies',
    unitMiles: 'millas',
    mAhead: '{{distance}} por delante',
    mAway: '{{distance}}',
    confirmed_one: '{{count}} persona ha confirmado',
    confirmed_other: '{{count}} personas han confirmado',
  },

  // ── Error Boundary ──
  errorBoundary: {
    title: 'Algo ha salido mal',
    description: 'La aplicación ha encontrado un error inesperado. Puedes intentarlo de nuevo o reiniciar la app.',
    tryAgain: 'Intentar de nuevo',
    restartApp: 'Reiniciar app',
  },

  // ── Tabs ──
  tabs: {
    map: 'Mapa',
    history: 'Historial',
    community: 'Comunidad',
    profile: 'Perfil',
  },

  // ── Route Planning ──
  planning: {
    searchDestination: '¿A dónde vas?',
    addStop: 'Añadir parada',
    previewRoute: 'Vista previa',
    enterDestinationFirst: 'Introduce un destino para continuar',
    safe: 'Seguro',
    fast: 'Rápido',
    flat: 'Llano',
    cool: 'Fresco',
    ebike: 'E-bike',
    ebikeA11y: 'Ruta para bicicleta eléctrica — subidas más fáciles y tiempos realistas',
    searchPlaceholder: '¿A dónde vamos?',
    searchPlaceholderOffline: 'Conéctate a internet para buscar',
    searchStartPlaceholder: 'Busca otro punto de partida',
    searchStopPlaceholder: 'Busca una parada',
    customStartLabel: 'Salida personalizada',
    hazardDescribeTitle: 'Describe el peligro',
    hazardDescribeSubtitle: 'Opcional — una nota breve ayuda a otros ciclistas.',
    hazardDescribePlaceholder: 'p. ej. perro suelto, cristales, puerta cerrada…',
    savedRoutes: 'Rutas guardadas',
    reportHazard: 'Reportar peligro',
    centerLocation: 'Centrar en mi ubicación',
    coverageOriginUnsupported:
      'La ruta segura aún no está disponible en tu zona — usamos rutas ciclistas estándar.',
    coverageDestinationUnsupported:
      'Tu destino está fuera de la cobertura de rutas seguras — usamos rutas ciclistas estándar.',
    riskDataUnavailable:
      'El enrutamiento seguro elige aquí calles tranquilas y aptas para bicis. Las puntuaciones de riesgo calle a calle aún no están activas en tu país.',
  },

  // ── Route Preview ──
  preview: {
    unpavedNotSupported: 'Las rutas rápidas no pueden evitar el firme sin asfaltar — cambia a Segura.',
    noPavedRoute: 'Aquí no hay ninguna ruta totalmente asfaltada — esta tiene tramos sin asfaltar.',
    modeCycleA11y: 'Ruta: {{current}}. Toca para cambiar a {{next}}.',
    modeCycleHint: 'Recalcula la ruta con el nuevo perfil',
    recomputingA11y: 'Recalculando la ruta',
    modeRoutingLong: 'Ruta {{mode}}',
    startNavigation: 'Iniciar navegación',
    returnToNavigation: 'Volver a la navegación',
    switchToSafe: 'Cambiar a ruta segura',
    switchingToSafe: 'Cambiando a ruta segura...',
    saveRoute: 'Guardar ruta',
    saveRouteTitle: 'Guardar ruta',
    saveRouteA11y: 'Guardar esta ruta',
    saving: 'Guardando...',
    dismissSaveRouteA11y: 'Cerrar diálogo de guardar',
    exportGpxA11y: 'Exportar la ruta como archivo GPX',
    exportGpxDialogTitle: 'Exportar ruta GPX',
    exportGpxFailed: 'No se pudo exportar el archivo GPX. Inténtalo de nuevo.',
    exportGpxOffline: 'Sin conexión — exportar una ruta guardada requiere calcularla primero.',
    exportDestinationTitle: 'Exportar GPX',
    exportToGarmin: 'Enviar a Garmin Connect',
    exportShareFile: 'Guardar o compartir archivo',
    gpxRouteName: 'Ruta Defensive Pedal',
    gpxStart: 'Inicio',
    gpxDestination: 'Destino',
    gpxVia: 'Vía',
    riskDistribution: 'Distribución del riesgo',
    elevationProfile: 'Perfil de altitud',
    routeComparison: 'Comparativa de rutas',
    previewLoading: 'Cargando vista previa',
    previewFailed: 'Error en la vista previa',
    previewFailedBody: 'No pudimos calcular esta ruta. Revisa tu conexión e inténtalo de nuevo.',
    noRoutes: 'No hay rutas disponibles',
    noRoutesBody: 'El servidor no ha devuelto alternativas para esta solicitud. Prueba el modo rápido o un destino diferente.',
    noRouteSelected: 'Ninguna ruta seleccionada',
    backToPlanning: 'Volver a planificación',
    retryPreview: 'Reintentar vista previa',
    routeName: 'Nombre de la ruta (p. ej. Camino al trabajo)',
    peekSwipeUp: 'Desliza arriba',
    lifeEarnedShort: 'de vida ganada',
    missingConfigTitle: 'Falta configuración',
    missingConfigBody: 'Configura `EXPO_PUBLIC_MOBILE_API_URL` antes de pedir vistas previas.',
    // Offline download card
    availableOffline: 'Disponible sin conexión',
    downloadingPct: 'Descargando... {{percent}}%',
    downloadFailed: 'Error al descargar',
    downloadForOffline: 'Descargar para sin conexión',
    downloadForOfflineA11y: 'Descargar ruta para uso sin conexión',
    // Safe-vs-fast comparison badge
    comparison: {
      calmerCost: '+{{minutes}} min por una ruta más tranquila',
      calmerCostSub: 'Un poco más de distancia a cambio de mucho menos tráfico.',
      calmerFree: 'Más tranquila que la ruta rápida — sin tiempo extra',
      busySaving: '{{distance}} menos por vías con mucho tráfico que la ruta rápida.',
      similarFast: 'Seguridad similar a la ruta rápida',
      sameFast: 'La misma seguridad que la ruta rápida',
      lessSafe: 'Un {{percent}}% menos segura que la ruta segura',
      slightlyLessSafe: 'Algo menos segura que la ruta segura',
      similarSafe: 'Seguridad similar a la ruta segura',
      sameSafe: 'La misma seguridad que la ruta segura',
    },
    // Porcentajes ABSOLUTOS, nunca una diferencia — la ruta con sombra puede
    // puntuar por debajo de la estándar. La afirmación es sobre arbolado, no
    // sobre temperatura: nunca «X grados más fresco».
    canopy: {
      moreShade: '+{{points}}% más de sombra',
      moreShadeSub:
        'El {{shade}}% de esta ruta va bajo arbolado, frente al {{standard}}% de la ruta estándar.',
      similar: 'Arbolado similar al de la ruta estándar',
      similarSub: 'El {{shade}}% de esta ruta va bajo arbolado.',
      comparison:
        'Ruta con sombra: {{shade}}% bajo arbolado · Ruta estándar: {{standard}}% bajo arbolado',
      comparisonA11y:
        'Ruta con sombra: {{shade}} por ciento bajo arbolado. Ruta estándar: {{standard}} por ciento bajo arbolado.',
    },
  },

  // ── Rutas GPX importadas ──
  plusModes: {
    promo: {
      title: 'Las rutas Fresco y E-bike están abiertas a todos',
      body: 'Fresco te lleva bajo los árboles siempre que podamos. E-bike calcula las cuestas pensando en un motor. Ambas son gratis para todos los ciclistas hasta el {{date}}.',
      afterwards: 'Después pasarán a formar parte de Plus.',
      cta: 'Entendido',
    },
  },

  course: {
    importCta: 'Importar GPX',
    importCtaA11y: 'Importar una ruta desde un archivo GPX',
    tabSaved: 'Guardadas',
    tabCourses: 'Importadas',
    badge: 'Ruta',
    reviewTitle: 'Ruta importada',
    namePlaceholder: 'Nombre de la ruta',
    nameFallback: 'Ruta importada',
    startRide: 'Empezar ruta',
    saveCourse: 'Guardar ruta',
    saved: 'Ruta guardada en este dispositivo.',
    savedAlready: 'Guardada',
    errorSave: 'No se ha podido guardar la ruta en el dispositivo.',
    openFailed: 'No se ha podido abrir la ruta. Puede que se haya eliminado.',
    deleteCourse: 'Eliminar ruta',
    emptyCourses: 'Aún no hay rutas importadas.',
    reading: 'Leyendo el archivo…',
    scoring: 'Analizando la ruta…',
    distance: 'Distancia',
    climb: 'Desnivel',
    estTime: 'Tiempo est.',
    notOurRoute: 'Tú elegiste esta ruta — no hemos elegido nosotros estas calles. Aquí es donde se complica.',
    turnsOnly: 'Solo direcciones — una ruta importada no incluye nombres de calles.',
    busyTitle: 'Tramos con tráfico',
    busyNone: 'Ningún tramo de riesgo alto en esta ruta.',
    busyHint: 'Toca un tramo para verlo en el mapa.',
    busyFound_one: '{{count}} tramo con tráfico',
    busyFound_other: '{{count}} tramos con tráfico',
    busyRowLabel: '{{distance}} en {{category}}',
    atKm: 'a {{distance}}',
    multipleTracks_one: 'El archivo tenía 1 ruta más — hemos importado la más larga.',
    multipleTracks_other: 'El archivo tenía {{count}} rutas — hemos importado la más larga.',
    droppedPoints_one: 'Se ha omitido 1 punto corrupto.',
    droppedPoints_other: 'Se han omitido {{count}} puntos corruptos.',
    downsampled: 'Hemos simplificado un trazado muy detallado para que el mapa siga fluido.',
    errorNotGpx: 'Este archivo no es un GPX válido.',
    errorNoTrack: 'Este archivo no contiene ninguna ruta — solo waypoints.',
    errorTooFewPoints: 'Esta ruta tiene muy pocos puntos para seguirla.',
    errorRead: 'No se ha podido leer el archivo. Inténtalo de nuevo.',
    errorBuild: 'No se ha podido crear una ruta con este archivo.',
  },

  // ── Risk Score ──
  loop: {
    fabLabel: 'Buscar una ruta circular',
    title: 'Buscar una ruta circular',
    subtitle: 'Una ruta que te trae de vuelta aquí.',

    startLabel: 'Inicio',
    startCurrent: 'Ubicación actual',
    startChange: 'Empezar en otro sitio',
    startPickHint: 'Toca el mapa para mover el inicio.',
    startCustom: 'Punto elegido',
    distanceLabel: 'Distancia',
    distanceValue: '{{distance}}',
    timeEstimate: 'unos {{minutes}} min',
    timeEstimateFrom: 'unos {{minutes}} min · según {{rides}} rutas',
    terrainLabel: 'Terreno',
    terrainFlat: 'Llano',
    terrainRolling: 'Ondulado',
    terrainHilly: 'Montañoso',
    surfaceLabel: 'Superficie',
    surfacePaved: 'Solo asfalto',
    surfaceAny: 'Permitir sin asfaltar',
    surfaceOffroad: 'Preferir sin asfaltar',
    surfaceOffroadNote: 'No podemos pedirle al router que busque grava, así que generamos rutas y elegimos la que más tiene. Cada resultado muestra cuánta encontró.',
    unpavedShare: '{{percent}}% sin asfaltar',
    retraced: 'Repite el {{percent}}% del recorrido',
    placementLabel: 'Dónde',
    placementOutOfTown: 'Fuera de la ciudad',
    placementAroundHere: 'Por aquí cerca',
    placementTooShort: '{{distance}} es la ruta más corta que te saca de la ciudad desde aquí. Las vueltas más cortas se quedan dentro.',
    placementNoStepClears: 'Ni la ruta más larga que ofrecemos sale de la ciudad desde aquí. Empieza más cerca del borde de la ciudad.',
    reach: 'Llega a {{distance}}',
    headingLabel: 'Hacia dónde',
    headingAny: 'Cualquier dirección',
    headingN: 'Norte',
    headingNE: 'Noreste',
    headingE: 'Este',
    headingSE: 'Sureste',
    headingS: 'Sur',
    headingSW: 'Suroeste',
    headingW: 'Oeste',
    headingNW: 'Noroeste',
    headingDialA11y: 'Dirección de la ruta, ahora {{direction}}. Arrastra para cambiar.',

    findCta: 'Buscar rutas',
    searching: 'Buscando rutas…',
    searchProgress: '{{found}} de {{total}}',
    cancel: 'Cancelar',
    tryAnother: 'Probar otras',

    resultsTitle: '{{count}} rutas',
    sessionCount: 'Encontradas en esta sesión: {{count}}',
    climb: '{{distance}} de desnivel',
    statDistance: 'Distancia',
    statTime: 'Tiempo est.',
    statClimb: 'Desnivel',
    save: 'Guardar ruta',
    saved: 'Ruta guardada.',
    savedAlready: 'Guardada',
    errorSave: 'No se pudo guardar esta ruta.',
    startRide: 'Empezar ruta',
    badge: 'Circular',
    nameFallback: 'Ruta circular de {{distance}}',
    namePattern: '{{distance}} hacia {{direction}}',

    relaxedHeading: 'Va hacia {{actual}} — no hay nada {{terrain}} al {{asked}}.',
    relaxedDistance: '{{distance}} — ninguna ruta de {{asked}} exactos.',
    relaxedRetrace: 'Toda ruta por aquí repite parte de sí misma. Esta repite el {{percent}}% — lo menos que encontramos.',
    relaxedTerrain: 'Nada {{asked}} por aquí. Revisamos {{checked}} rutas; esta es la más cercana.',

    emptyTitle: 'Ninguna ruta de {{distance}} desde aquí',
    emptyBody: 'Las calles de esta zona no se cierran en un circuito.',
    emptyTryLonger: 'Probar {{distance}}',
    emptyTryDirection: 'Probar otra dirección',
    emptyTryElsewhere: 'Empezar en otro sitio',
    offline: 'Buscar rutas necesita conexión.',
    errorTitle: 'La búsqueda ha fallado',
    errorBody: 'Ha sido un problema nuestro, no de tus carreteras. Inténtalo otra vez.',
    errorOfflineBody: 'Vuelve a conectarte e inténtalo de nuevo.',
    errorRetry: 'Intentar de nuevo',

    unavailableTitle: 'Las rutas circulares aún no están disponibles aquí',
    unavailableBody: 'Necesitan nuestros datos de riesgo vial. Cubren 32 países — este todavía no.',
    unavailableCta: 'Únete a la lista de espera',

    sessionsLeft: 'Te quedan {{count}} de {{total}} sesiones este mes',
    sessionActive: 'La sesión sigue abierta {{minutes}} min',
    quotaTitle: 'Has usado tus sesiones de rutas',
    quotaBody: 'Los ciclistas gratuitos tienen {{total}} sesiones de búsqueda al mes. Se reinician el mes que viene.',

    sectionTitle: 'Rutas circulares',
    emptyLoops: 'Aún no hay rutas guardadas.',
    share: 'Compartir ruta',
    saveLimit: 'En el plan gratuito puedes guardar {{count}} rutas. Elimina una para guardar esta.',
    duration: '{{minutes}} min',
    statTimeAtPace: 'Tiempo estimado a {{speed}}',
    roadsLabel: 'Carreteras',
    roadsBusy: '{{distance}} por vías con tráfico',
    roadsNoneBusy: 'Sin vías con tráfico',
    surfaceSplit: '{{paved}}% asfalto / {{unpaved}}% sin asfaltar',
    spurNote: 'Incluye {{distance}} de desvío de ida y vuelta',
    noPavedLoop: 'Aquí no hay ninguna ruta circular asfaltada — esta tiene tramos sin asfaltar',
    deleteLoop: 'Eliminar ruta',
    driftNotice: 'Esta ruta ha cambiado desde que la guardaste — ahora {{now}}, antes {{before}}.',
  },
  risk: {
    cardTitle: 'Riesgo de la ruta',
    cardInfoA11y: 'Cómo funciona la Puntuación de Riesgo',
    takeawayMostlyGreen: 'Mayormente por calles seguras — el {{percent}}% de esta ruta va por verde.',
    takeawayNeutral: 'El {{percent}}% de esta ruta va por calles tranquilas.',
    busyStretch:
      '{{distance}} por una vía con mucho tráfico — aquí no hay alternativa más tranquila. Extrema la precaución en este tramo o camina con la bici donde esté permitido.',
    dataUnavailable:
      'Las puntuaciones de riesgo calle a calle aún no están disponibles en esta zona — por ahora cubren 32 países europeos (la UE, el EEE, Suiza y el Reino Unido).',
    explainer: {
      title: 'Cómo puntuamos cada calle',
      intro:
        'Cada carretera, camino y carril de la app lleva una Puntuación de Riesgo — nuestra estimación de cuánto te expone una calle al peligro cuando la recorres. Cuanto más baja, más segura. Las puntuaciones calle a calle cubren 32 países europeos — la UE, el EEE, Suiza y el Reino Unido.',
      anchor:
        'Una calle residencial tranquila queda hacia la mitad de la escala. Las calles por debajo se sienten más calmadas que tu calle de casa; las de arriba, más transitadas y exigentes.',
      legendTitle: 'Qué significan los colores',
      factorsTitle: 'Qué determina la puntuación',
      factorSeparation: 'Separación de los coches — carriles protegidos, carril bici o nada. Es lo que más pesa.',
      factorTraffic: 'Velocidad y tamaño del tráfico — límites de velocidad, número de carriles, vehículos pesados.',
      factorVolume: 'Volumen de tráfico — cuántos vehículos usan realmente la calle, incluidas las calles-atajo.',
      factorDesign: 'Diseño de la calle — rotondas, túneles, vías de tranvía, coches aparcados que pueden abrir la puerta.',
      factorSurface: 'Superficie — calidad del pavimento, adoquines, gravilla, badenes.',
      factorEnvironment: 'Entorno — iluminación nocturna, zonas escolares, obras.',
      routing:
        'Al crear una ruta, equilibramos seguridad y distancia — añadimos con gusto algo de longitud para mantenerte en calles tranquilas.',
      disclaimer:
        'La puntuación describe la calle, no el momento — no puede ver el tiempo de hoy ni cómo se comporta el tráfico ahora mismo. Circula siempre según lo que tengas delante.',
      attribution: 'Basado en datos de OpenStreetMap, modelos de elevación y modelado de tráfico.',
    },
    bands: {
      rampA11y: 'Categoría {{band}}, mostrada en el mapa con {{count}} tonos.',
      safer: {
        label: 'Más segura',
        desc: 'Carriles protegidos, vías verdes y calles realmente tranquilas — lo más cómodo para pedalear de la zona.',
      },
      typical: {
        label: 'Normal',
        desc: 'Calles corrientes con tráfico mixto de todos los días. Aquí está la mayor parte de la red.',
      },
      highRisk: {
        label: 'Riesgo alto',
        desc: 'Carreteras con mucho tráfico y velocidad. Un accidente aquí tiene más probabilidades de ser grave, por eso tu ruta las evita siempre que hay una alternativa más tranquila.',
      },
      noData: {
        label: 'Sin datos',
        desc: 'Esta calle aún no tiene puntuación.',
      },
    },
  },

  // ── Navigation ──
  nav: {
    noActiveTitle: 'Sin navegación activa',
    noActiveBody: 'Elige un destino y previsualiza una ruta para empezar a navegar.',
    returnToPreview: 'Volver a la vista previa',
    arrived: 'Has llegado a tu destino.',
    inMeters: 'En {{distance}}, {{instruction}}',
    thenInMeters: ', después en {{distance}}, {{instruction}}',
    minutesRemaining_one: 'Queda alrededor de 1 minuto.',
    minutesRemaining_other: 'Quedan alrededor de {{count}} minutos.',
    endRide: 'Terminar ruta',
    endRideConfirmTitle: '¿Terminar la ruta?',
    endRideConfirmMessage: '¿Seguro que quieres terminar la ruta? Tu progreso se guardará.',
    endRideSaveTitle: '¿Guardar esta ruta?',
    endRideSaveMessage: 'Guárdala en tu historial o descártala. Si la descartas, no se registra impacto, XP ni insignias.',
    endRideSave: 'Guardar',
    endRideDiscard: 'Descartar',
    endRideKeepRiding: 'Seguir pedaleando',
    resumeGuardTitle: '¿Terminar tu última ruta?',
    resumeGuardMessage: 'Ibas hacia {{destination}}. Reanuda la navegación o guarda en tu historial lo que pedaleaste.',
    resumeGuardResume: 'Reanudar',
    resumeGuardSave: 'Guardar ruta',
    resumeGuardDiscard: 'Descartar ruta',
    resumeGuardDestinationFallback: 'tu destino',
    bgServiceTitle: 'Navegación de Defensive Pedal activa',
    bgServiceBody: 'Registramos tu ruta para que la navegación siga funcionando con la pantalla apagada.',
    bgRecordingUnavailable:
      'La grabación con la pantalla apagada no está disponible: permite la ubicación «Todo el tiempo» en Ajustes o mantén la pantalla encendida para registrar toda tu ruta.',
    dismiss: 'Cerrar',
    earlyEndTitle: '¿Por qué has terminado la ruta antes de tiempo?',
    earlyEndReachedDestination: 'He llegado a mi destino',
    earlyEndFoundBetterRoute: 'He encontrado una ruta mejor por mi cuenta',
    earlyEndFeltUnsafe: 'La ruta sugerida me parecía insegura',
    earlyEndNoLongerNeeded: 'Simplemente ya no necesitaba guía',
    earlyEndOther: 'Otro',
    earlyEndOtherPlaceholder: 'Cuéntanos por qué terminaste antes…',
    earlyEndSave: 'Guardar ruta',
    earlyEndDiscard: 'Descartar ruta',
    earlyEndSkip: 'Omitir',
    recenter: 'Recentrar',
    offRoute: 'Estás fuera de ruta',
    offCourse: 'Has salido de la ruta — vuelve al trazado.',
    offCourseDistance: 'Fuera de la ruta — {{distance}} hasta el trazado.',
    offRouteCountdown: 'Fuera de ruta. Recálculo automático en {{seconds}}s.',
    offRouteReady: 'Fuera de ruta. Recálculo manual disponible.',
    rerouting: 'Solicitando una ruta nueva desde tu posición GPS.',
    rerouteFailed: 'No pudimos recalcular la ruta. Toca para reintentar.',
    rerouteNow: 'Recalcular ahora',
    retryGps: 'Reintentar GPS',
    gpsLive: 'GPS activo',
    syncOn: 'Sincronización activa',
    anonymousRide: 'Ruta anónima',
    step: 'Paso {{current}}/{{total}}',
    direction: {
      left: 'a la izquierda',
      right: 'a la derecha',
      straight: 'recto',
      slightLeft: 'ligeramente a la izquierda',
      slightRight: 'ligeramente a la derecha',
      sharpLeft: 'cerrado a la izquierda',
      sharpRight: 'cerrado a la derecha',
    },
    maneuver: {
      theRoad: 'la vía',
      depart: 'Avanza {{direction}} por {{street}}',
      departNoDir: 'Comienza por {{street}}',
      arrive: 'Has llegado a tu destino',
      uturn: 'Haz un cambio de sentido hacia {{street}}',
      turn: 'Gira {{direction}} hacia {{street}}',
      continue: 'Continúa por {{street}}',
      merge: 'Incorpórate a {{street}}',
      fork: 'Mantente {{direction}} hacia {{street}}',
      roundabout: 'Entra en la rotonda hacia {{street}}',
      rampOn: 'Toma la rampa hacia {{street}}',
      rampOff: 'Toma la salida hacia {{street}}',
      generic: 'Continúa {{direction}} hacia {{street}}',
    },
    maneuverShort: {
      continue: 'Continúa',
      depart: 'Inicio',
      arrive: 'Llegada',
      roundabout: 'Rotonda',
      roundaboutExit: 'Salida {{exit}}',
      slightLeft: 'Leve izquierda',
      slightRight: 'Leve derecha',
      sharpLeft: 'Cerrado izq.',
      sharpRight: 'Cerrado der.',
      turnLeft: 'Gira izquierda',
      turnRight: 'Gira derecha',
      uturn: 'Cambio de sentido',
    },
    then: 'Luego',
    etaNow: 'Ahora',
    metricSpeed: 'Velocidad',
    metricEta: 'Llegada',
    metricDist: 'Dist.',
    metricClimb: 'Subida',
    // Navegación con varias paradas
    stopXofN: 'Parada {{index}} de {{count}}',
    toFinish: 'hasta el final: {{dist}} · {{eta}}',
    skipStop: 'Saltar parada',
    skipStopCancel: 'Mantener',
    skipStopConfirmTitle: '¿Saltar esta parada?',
    skipStopConfirmBody: '¿Saltar la parada {{index}} y recalcular hacia la siguiente?',
    guardTitle: 'Navegar',
    guardEyebrow: 'Trayecto activo',
    guardSubtitle: 'Empieza desde la vista previa de la ruta para entrar en la navegación paso a paso.',
    offlineFollowRoute: 'Sin conexión — sigue la ruta planificada',
    hazardGpsUnavailable: 'No se puede reportar el peligro porque el GPS no está disponible.',
    hazardRecordedSync: '{{label}} registrado, se sincronizará automáticamente.',
    hazardRecordedAnon: '{{label}} registrado. Se sincronizará de forma anónima cuando la API esté disponible.',
    freeMap: 'Mapa libre',
    reportHazard: 'Reportar peligro',
    showElevation: 'Mostrar altitud',
    hideElevation: 'Ocultar altitud',
    maneuverA11y: '{{description}}, en {{distance}}',
    tapReplay: 'Toca para escuchar la indicación de nuevo',
    offlineNoInternet: 'Sin conexión — sin internet',
    gpsSignal: 'Señal GPS {{quality}}',
    gpsUnavailable: 'no disponible',
    gpsStrong: 'fuerte',
    gpsFair: 'aceptable',
    gpsPoor: 'débil',
    steepLabel: 'Pendiente',
    steepUphill: 'en subida',
    steepDownhill: 'en bajada',
    steepGradeA11y: 'Pendiente pronunciada {{direction}}: {{percent}} por ciento',
  },

  // ── Feedback ──
  feedback: {
    title: '¿Qué tal la ruta?',
    subtitle: 'Tus comentarios nos ayudan a mejorar las rutas',
    submit: 'Enviar comentario',
    skip: 'Omitir',
    greatRide: '¡Buena ruta!',
    positiveImpact: 'Este es el impacto positivo de tu trayecto.',
    continue: 'Continuar',
    howSafe: '¿Qué tan segura fue tu ruta?',
    safetyHelps: 'Tus comentarios ayudan a mejorar las rutas futuras.',
    perceivedSafety: 'Seguridad percibida',
    commentsOptional: 'Comentarios (opcional)',
    commentsPlaceholder: '¿Algún comentario sobre la ruta?',
    thankYou: '¡Gracias!',
    thankYouSub: 'Tus comentarios hacen que las calles sean más seguras para todos.',
    milestone: '¡Has alcanzado un hito!',
    shareAchievement: 'Comparte tu logro',
    maybeLater: 'Quizá más tarde',
    saveProgress: 'Guarda tu progreso',
    signUpKeepStreak: 'Crea una cuenta para mantener tu racha y seguir tu impacto.',
    continueGoogle: 'Continuar con Google',
    skipConfirmTitle: '¿Omitir comentario?',
    skipConfirmMessage: 'Tus comentarios ayudan a mejorar la seguridad de las rutas para todos los ciclistas.',
    skipConfirmButton: 'Omitir',
  },

  // ── Review Prompt (Play Store sentiment funnel) ──
  reviewPrompt: {
    sentimentTitle: '¿Te gusta Defensive Pedal?',
    sentimentBody: 'Tu respuesta sincera nos ayuda a decidir qué construir a continuación.',
    sentimentPositive: 'Me encanta',
    sentimentNegative: 'Podría mejorar',
    sentimentLater: 'Luego',
    positiveTitle: '¿Nos dejas una reseña en Play Store?',
    positiveBody: '30 segundos. Ayuda a otros ciclistas a descubrir la app.',
    positiveCta: 'Claro, dejaré una reseña',
    positiveDismiss: 'Quizá más tarde',
    negativeTitle: 'Lamentamos oír eso — ¿qué falta?',
    negativeBody: 'Dínoslo directamente. Leemos cada respuesta.',
    negativeCta: 'Enviar comentarios',
    negativeDismiss: 'Omitir',
    thanksTitle: '¡Gracias por tu opinión!',
    thanksBody: 'Significa mucho.',
    dismissA11y: 'Cerrar petición de reseña',
  },

  // ── Save-ride signup card (impacto post-ruta, solo usuarios anónimos) ──
  saveRide: {
    title: 'Guarda esta ruta antes de que desaparezca',
    body: 'Tu ruta, XP e insignias solo viven en este teléfono. Un toque las guarda para siempre.',
    keepGuest: 'Sigo pedaleando como invitado',
    dismissA11y: 'Cerrar el aviso de guardar la ruta',
    successToast: 'Sesión iniciada — tus rutas ahora se sincronizan con tu cuenta.',
    error: 'El inicio de sesión no terminó — tu ruta sigue aquí. Inténtalo cuando quieras.',
  },

  // ── Product-analytics opt-in prompts (docs/plans/analytics-optin-prompts.md) ──
  analyticsOptIn: {
    successToast: 'Gracias — ahora ayudas a definir el rumbo.',
    noThanks: 'No, gracias',
    dismissA11y: 'Cerrar el aviso de analíticas',
    footer: 'Anónimo. Sin rutas GPS. Desactívalo cuando quieras en Perfil › Privacidad.',
    p1Title: 'Ayuda a decidir qué construimos',
    p1Body: 'Dos rutas ya — está claro que le pillas el punto. Comparte estadísticas anónimas y las funciones que más usas se construyen primero.',
    p1Cta: 'Cuenta conmigo',
    p2Title: 'Acabas de hacer el mapa más seguro',
    p2Body: 'Ese aviso ayuda a ciclistas cercanos. ¿Nos ayudas igual? Las estadísticas anónimas nos muestran qué funciona.',
    p2Cta: 'Compartir estadísticas anónimas',
    p3Title: 'Tú sigues tus estadísticas. ¿Nos dejas seguir las nuestras?',
    p3Body: 'Los eventos anónimos de uso nos dicen qué funciones de seguridad se usan de verdad — para apostar por las correctas.',
    p3Cta: 'Activar analíticas',
  },

  // ── Profile ──
  profile: {
    title: 'Perfil',
    eyebrow: 'Defensive Pedal',
    subtitle: 'Tu cuenta y ajustes',
    sectionCycling: 'Preferencias de ciclismo',
    sectionDisplay: 'Pantalla',
    sectionAccount: 'Cuenta',
    tapToSignIn: 'Toca para iniciar sesión',
    signInOrUp: 'Inicia sesión o crea una cuenta',
    signInOrUpSub: 'Guarda tus rutas, insignias y progreso',
    signOut: 'Cerrar sesión',
    setUsername: 'Establecer nombre de usuario',
    changeUsername: 'Cambiar nombre de usuario',
    minChars: 'Mínimo 3 caracteres',
    usernameTaken: 'Nombre de usuario ya en uso',
    language: 'Idioma',
    aboutYou: 'Sobre ti',
    bikeType: 'Tipo de bicicleta',
    cyclingFrequency: '¿Con qué frecuencia montas?',
    bodyWeight: 'Peso corporal',
    bodyWeightDesc: 'Usado para calcular las calorías quemadas',
    bodyWeightUnit: 'kg',
    routingPreferences: 'Preferencias de ruta',
    avoidUnpaved: 'Evitar caminos sin asfaltar',
    avoidHills: 'Evitar cuestas',
    compareRoutes: 'Comparar ruta segura vs rápida',
    showBikeLanes: 'Mostrar carriles bici en el mapa',
    showRouteFeatures: 'Mostrar elementos de la ruta',
    useMiles: 'Distancias en millas',
    showMascot: 'Mostrar a Pedal',
    themeDark: 'Oscuro',
    themeLight: 'Claro',
    themeSystem: 'Sistema',
    quizRegion: 'Región del cuestionario',
    quizRegionAuto: 'Automático',
    quizRegionRO: 'Rumanía',
    quizRegionES: 'España',
    quizRegionSubtitleAuto: 'Automático · detectado: {{country}}',
    quizRegionSubtitleOverride: 'Fijado en {{country}}',
    quizRegionRomania: 'Rumanía',
    quizRegionSpain: 'España',
    quizRegionGeneric: 'General (Europa)',
    pointsOfInterest: 'Puntos de interés',
    notifications: 'Notificaciones',
    dailyWeather: 'Tiempo para ciclismo',
    hazardAlerts: 'Alertas de peligro',
    community: 'Comunidad',
    quietHours: 'Horas tranquilas',
    privacy: 'Privacidad',
    shareTrips: 'Compartir rutas públicamente',
    guardianTier: 'Nivel guardián',
    // Toggle descriptions
    avoidUnpavedOn: 'Las rutas se quedarán en superficies asfaltadas',
    avoidUnpavedOff: 'Las rutas pueden incluir caminos sin asfaltar',
    avoidHillsOn: 'Las rutas preferirán caminos más llanos — puede añadir distancia',
    avoidHillsOff: 'Las rutas usan el camino seguro más directo',
    compareRoutesOn: 'Muestra cuánto más segura es tu ruta frente a la rápida',
    compareRoutesOff: 'Comparativa de rutas desactivada',
    showBikeLanesOn: 'La infraestructura ciclista se muestra en el mapa',
    showBikeLanesOff: 'Los carriles bici están ocultos en el mapa',
    useMilesOn: 'Millas, pies y mph',
    useMilesOff: 'Kilómetros, metros y km/h',
    showRouteFeaturesOn: 'Túneles, puentes, semáforos y pasos a nivel se muestran en el mapa',
    showRouteFeaturesOff: 'Los marcadores de elementos de la ruta están ocultos',
    showMascotOn: 'Pedal aparece en momentos amistosos',
    showMascotOff: 'Pedal está oculto en toda la app',
    reviewPrompts: 'Peticiones de reseña en Play Store',
    reviewPromptsOn: 'Te invitaremos ocasionalmente a valorar la app tras rutas exitosas',
    reviewPromptsOff: 'No se te pedirá valorar la app',
    dailyWeatherOn: 'Pronóstico de ciclismo y recordatorios, de vez en cuando',
    dailyWeatherOff: 'Las notificaciones del tiempo están desactivadas',
    hazardAlertsOn: 'Recibe notificaciones sobre peligros cerca de tus rutas',
    hazardAlertsOff: 'Las alertas de peligro están desactivadas',
    communityOn: 'Recibe notificaciones sobre me gusta y comentarios',
    communityOff: 'Las notificaciones de la comunidad están desactivadas',
    quietHoursDesc: 'Sin notificaciones {{start}} → {{end}}',
    quietHoursStart: 'Inicio de las horas tranquilas',
    quietHoursEnd: 'Fin de las horas tranquilas',
    quietHoursOff: 'El inicio y el fin coinciden, así que las horas tranquilas están desactivadas — las notificaciones pueden llegar en cualquier momento.',
    quietHoursOvernight: 'Intervalo nocturno — continúa pasada la medianoche, hasta el día siguiente.',
    shareTripsOn: 'Tus rutas se comparten en el feed de la comunidad',
    shareTripsOff: 'Tus rutas son privadas y no se comparten',
    // User card
    rider: 'Ciclista',
    // Sign out
    signOutConfirm: '¿Seguro que quieres cerrar sesión?',
    selectBikeType: 'Selecciona tu tipo de bicicleta',
    howOften: '¿Con qué frecuencia montas?',
    bikeRoad: 'Bicicleta de carretera',
    bikeCity: 'Bicicleta de ciudad',
    bikeMountain: 'Bicicleta de montaña',
    bikeEbike: 'Bicicleta eléctrica',
    bikeRecumbent: 'Bicicleta reclinada',
    bikeOther: 'Otra',
    freqDaily: 'A diario',
    freqSeveralWeek: 'Varias veces por semana',
    freqOnceWeek: 'Una vez por semana',
    freqFewMonth: 'Algunas veces al mes',
    freqOnceMonth: 'Una vez al mes',
    freqRarely: 'Menos de una vez al mes',
    poiWater: 'Agua y cafeterías',
    poiWaterDesc: 'Fuentes y cafeterías',
    poiRepair: 'Tiendas de bicis',
    poiRepairDesc: 'Reparación y tiendas de bicicletas',
    poiRental: 'Alquiler de bicis',
    poiRentalDesc: 'Estaciones de alquiler de bicicletas',
    poiParking: 'Aparcamiento de bicis',
    poiParkingDesc: 'Sitios para aparcar la bici',
    poiRestroom: 'Aseos',
    poiRestroomDesc: 'Aseos públicos',
    poiSupplies: 'Tiendas',
    poiSuppliesDesc: 'Tiendas de conveniencia, alimentación',
    rebuildRequired: 'Reconstrucción necesaria',
    rebuildMessage: 'El selector de fotos requiere una reconstrucción nativa.',
    photoUploadFailed: 'Error al subir la foto',
    // Delete account
    deleteAccount: 'Eliminar cuenta',
    deleteAccountSub: 'Elimina permanentemente tu cuenta y todos tus datos',
    // Compliance plan item 13: GPS retention opt-out
    keepFullGpsHistory: 'Conservar el historial GPS completo',
    keepFullGpsHistoryOn: 'Las trazas GPS sin procesar se conservan para cada ruta hasta que elimines tu cuenta.',
    keepFullGpsHistoryOff: 'Los resúmenes de las rutas se conservan para siempre; las trazas GPS sin procesar se eliminan automáticamente tras 90 días. La distancia, CO2 e insignias no se ven afectados.',
    // ── Pedal Nudges section ──
    sectionPedalNudges: 'Avisos de Pedal',
    nudgeMaster: 'Avisos de Pedal',
    nudgeMasterOn: 'Pedal puede enviarte recordatorios y celebraciones',
    nudgeMasterOff: 'Todos los avisos de Pedal están desactivados',
    pedalVoice: 'Voz de Pedal',
    pedalVoiceSassy: 'Con descaro',
    pedalVoiceNeutral: 'Neutra',
    pedalVoiceSassyDesc: 'Pedal pincha sin juzgar. Es la voz de la marca.',
    pedalVoiceNeutralDesc: 'Mensajes funcionales, sin bromas. Mismos avisos, frases sencillas.',
    nudgeCategoryStreak: 'Recordatorios de racha',
    nudgeCategoryStreakDesc: 'Racha en riesgo + disculpa tras perderla + hitos',
    nudgeCategoryStreakOff: 'Sin avisos relacionados con la racha',
    // Anonymous activation ladder (getting-started reminders)
    activationLadder: 'Recordatorios para empezar',
    activationLadderDesc: 'Hasta tres recordatorios de Pedal hasta tu primera ruta',
    activationLadderOff: 'Sin recordatorios para empezar',
    // ── Meet Pedal one-time onboarding card ──
    meetPedalTitle: 'Hola, soy Pedal',
    meetPedalBody: 'Lo llevo con ligereza — un empujoncito cuando tu racha está en riesgo, un grito de ánimo cuando terminas una ruta. Puedes bajarme el volumen desde Perfil si me pongo pesado.',
    meetPedalCta: 'Entendido',
    // ── Achievements / referral row ──
    achievementsLabel: 'Logros',
    badgesEarned: '{{earned}} / {{total}} insignias conseguidas',
    confidentCyclistTitle: 'Ciclista seguro',
    helpAFriend: 'Ayuda a un amigo a empezar',
    helpAFriendSub: 'Comparte Defensive Pedal con un amigo',
    referralShareMessage: 'Prueba Defensive Pedal — rutas más seguras para cualquier ciudad. {{url}} #DefensivePedal',
  },

  // ── Report sheet ──
  report: {
    title: 'Reportar contenido',
    subtitle: 'Ayúdanos a mantener la comunidad segura. Los reportes van a nuestro moderador y son privados.',
    reasonLabel: '¿Por qué lo reportas?',
    reasonSpam: 'Spam',
    reasonHarassment: 'Acoso',
    reasonHate: 'Discurso de odio',
    reasonSexual: 'Contenido sexual',
    reasonViolence: 'Violencia o amenazas',
    reasonIllegal: 'Actividad ilegal',
    reasonOther: 'Otro',
    detailsLabel: 'Detalles adicionales (opcional)',
    detailsPlaceholder: '¿Qué deberíamos saber sobre este reporte?',
    submit: 'Enviar reporte',
    successTitle: 'Reporte recibido',
    successMessage: 'Gracias — lo revisaremos en 48 horas.',
    errorTitle: 'No se pudo enviar el reporte',
    errorMessage: 'Por favor, inténtalo de nuevo en un momento.',
    alreadyReported: 'Ya has reportado este contenido.',
  },

  // ── Blocked users screen ──
  blockedUsers: {
    title: 'Usuarios bloqueados',
    profileRowSub: 'Gestiona los usuarios que has bloqueado del feed',
    loading: 'Cargando…',
    loadError: 'No se pudo cargar la lista de usuarios bloqueados.',
    emptyTitle: 'Ningún usuario bloqueado',
    emptyMessage: 'Cuando bloqueas a alguien, sus publicaciones, peligros y comentarios quedan ocultos aquí y en toda la app.',
    blockedAt: 'Bloqueado el {{date}}',
    unblock: 'Desbloquear',
    unblockConfirmTitle: '¿Desbloquear a este usuario?',
    unblockConfirmMessage: 'El contenido de {{name}} volverá a aparecer en tu feed.',
    errorTitle: 'No se pudo desbloquear',
    errorMessage: 'Por favor, inténtalo de nuevo en un momento.',
  },

  // ── Onboarding analytics consent ──
  onboardingConsent: {
    eyebrow: 'Defensive Pedal',
    title: 'Ayúdanos a mejorar, en tus términos',
    subtitle: 'Los reportes de fallos están activos por defecto — nos ayudan a corregir bugs más rápido. La analítica de producto está desactivada por defecto y es opcional.',
    crashLabel: 'Enviar reportes de fallos',
    crashDescription: 'Activo por defecto. Trazas de pila e información del dispositivo cuando la app falla — sin ubicación, sin datos personales. Nos ayuda a corregir errores más rápido. Puedes desactivarlo en cualquier momento.',
    analyticsLabel: 'Enviar analíticas de producto',
    analyticsDescription: 'Desactivado por defecto. Eventos de uso anónimos para entregar lo que importa. Sin seguimiento de ubicación. Actívalo si quieres ayudarnos a priorizar.',
    ridingTipsLabel: 'Consejos y recordatorios para pedalear',
    ridingTipsDescription: 'Notificaciones ocasionales que te ayudan a empezar a pedalear con seguridad. Puedes desactivarlas cuando quieras.',
    assurance: 'Tú tienes el control.',
    continue: 'Continuar',
    changeLater: 'Puedes cambiarlo en cualquier momento desde Perfil → Privacidad y analítica.',
  },

  // ── Privacy & Analytics settings screen ──
  privacyAnalytics: {
    title: 'Privacidad y analítica',
    profileRowSub: 'Preferencias para reportes de fallos y analítica de producto',
    intro: 'Los reportes de fallos y la analítica de producto están activos por defecto — los reportes mantienen la app estable y la analítica nos ayuda a decidir qué construir. Cambia cualquiera de los dos de forma independiente — desactivar un interruptor detiene los nuevos eventos para ese canal inmediatamente.',
    crashLabel: 'Enviar reportes de fallos',
    crashDescription: 'Activado por defecto para mantener la app estable. Anónimo. Desactívalo cuando quieras.',
    analyticsLabel: 'Enviar analíticas de producto',
    analyticsDescription: 'Activado por defecto. Eventos de uso anónimos que nos ayudan a decidir qué construir. Sin trazas GPS. Desactívalo cuando quieras.',
    crashUnavailable: 'Los reportes de fallos no están configurados en esta versión.',
    analyticsUnavailable: 'La analítica de producto no está configurada en esta versión.',
    assurance: 'Cuando un interruptor está apagado, no se envían eventos en ese canal. Nunca vendemos tus datos.',
  },

  // ── Delete account screen ──
  deleteAccount: {
    title: 'Eliminar cuenta',
    intro: 'Esta acción es permanente. Eliminaremos lo siguiente de nuestros servidores en 24 horas:',
    bulletTrips: 'Tu historial de rutas, trazas GPS y estadísticas de ciclismo',
    bulletShares: 'Las rutas que has compartido en el feed de la comunidad',
    bulletHazards: 'Los peligros que has reportado (el reporte se mantiene por la seguridad de la comunidad, pero tu nombre se elimina)',
    bulletSocial: 'Tus me gusta, me encanta, comentarios y relaciones de seguimiento',
    bulletProgress: 'Tu XP, insignias, racha y progreso de nivel',
    bulletProfile: 'Tu perfil, nombre de usuario, avatar y credenciales de cuenta',
    confirmInstruction: 'Para confirmar, escribe DELETE abajo.',
    confirmPlaceholder: 'Escribe DELETE',
    confirmHint: 'Escribe la palabra DELETE en mayúsculas para activar el botón de eliminar.',
    cancelButton: 'Cancelar',
    confirmButton: 'Eliminar mi cuenta',
    confirming: 'Eliminando…',
    successTitle: 'Cuenta eliminada',
    successMessage: 'Tu cuenta y todos los datos asociados han sido eliminados. Ahora se cerrará tu sesión.',
    errorTitle: 'Error al eliminar',
    errorMessage: 'No hemos podido eliminar tu cuenta. Por favor, inténtalo de nuevo o contacta con soporte si el problema persiste.',
    anonymousNotice: 'Estás conectado de forma anónima. No hay cuenta que eliminar — cerrar la app o borrar los datos de la app eliminará tu estado local.',
    signInRequired: 'Inicia sesión con Google para gestionar la eliminación de tu cuenta.',
  },

  // ── History ──
  history: {
    title: 'Historial',
    eyebrow: 'Defensive Pedal',
    subtitle: 'Tu viaje en bici',
    noTrips: 'Aún no hay rutas',
    yourImpact: 'Tu impacto',
    trips: 'Rutas',
    cycled: 'Pedaleado',
    totalRides: 'Rutas totales',
    totalDistance: 'Distancia total',
    totalDuration: 'Duración total',
    co2Saved: 'CO2 ahorrado',
    eurSaved: 'EUR ahorrados',
    hazards: 'Peligros',
    protected: 'Protegido',
    currentStreak: 'Racha actual',
    longestStreak: 'Mejor racha',
    exportGpx: 'Exportar GPX',
    exportGpxDialogTitle: 'Exportar recorrido GPX',
    gpxRideName: 'Recorrido Defensive Pedal',
    viewTrips: 'Ver mis rutas',
    dailyQuiz: 'Cuestionario diario de seguridad',
    dailyQuizSub: 'Pon a prueba tus conocimientos de ciclismo',
    rides: 'Rutas',
    km: 'km',
    streak: 'Racha',
    co2: 'CO2',
    noRidesYet: 'Aún no hay rutas',
    noRidesSub: 'Completa tu primera ruta y aparecerá aquí.',
    loadingTrips: 'Cargando rutas…',
    loadTripsFailed: 'Error al cargar las rutas.',
    allTrips: 'Todas las rutas',
    seeAll: 'Ver todo',
  },

  // ── Trips ──
  tripsScreen: {
    title: 'Historial',
    subtitle: 'Mis rutas',
    loading: 'Cargando tus rutas…',
    loadFailed: 'Error al cargar las rutas. Inténtalo de nuevo.',
    noRides: 'Aún no hay rutas',
    noRidesSub: 'Completa una ruta y tu historial aparecerá aquí.',
    completed: 'Completada',
    stopped: 'Detenida',
    appKilled: 'App cerrada',
    deleteAction: 'Eliminar ruta',
    deleteTitle: '¿Eliminar esta ruta?',
    deleteMessage: 'La ruta se quitará de tu historial y del feed de la comunidad. Los logros y totales de impacto se conservan.',
    deleteCancel: 'Cancelar',
    deleteConfirm: 'Eliminar',
    deleteFailed: 'No se pudo eliminar la ruta. Inténtalo de nuevo.',
    noGpsTrail: 'No hay traza GPS para esta ruta.',
    exportGpxA11y: 'Exportar el recorrido como archivo GPX',
  },

  // ── Community ──
  communityScreen: {
    title: 'Comunidad',
    subtitle: 'Conecta con otros ciclistas',
    feedTitle: 'Feed de la comunidad ciclista',
    feedSub: 'Mira las rutas que comparte la comunidad ciclista — primero los de tu zona.',
    viewFeed: 'Ver el feed de la comunidad',
    feed: 'Feed',
    noActivity: 'Sin actividad cerca',
    like: 'Me gusta',
    love: 'Me encanta',
    comment: 'Comentar',
    follow: 'Seguir',
    unfollow: 'Dejar de seguir',
    shareRide: 'Compartir ruta',
    locationNeeded: 'Necesitamos acceso a la ubicación',
    locationFailed: 'No se pudo obtener la ubicación',
    locationPrompt: 'Activa el permiso de ubicación para ver las rutas cercanas.',
    gettingLocation: 'Obteniendo tu ubicación...',
    grantLocation: 'Conceder acceso a la ubicación',
    tryAgain: 'Reintentar',
    loadFailed: 'No se pudo cargar el feed',
    tryAgainLater: 'Comprueba tu conexión e inténtalo de nuevo.',
    noRidesNearby: 'Aún no hay rutas compartidas cerca',
    beFirst: 'Completa una ruta y sé el primero en compartirla.',
    // Etiqueta de alcance ampliado del feed
    scopeRegion: 'Mostrando rutas de tu región',
    scopeCommunity: 'Mostrando rutas de toda la comunidad Defensive Pedal',
    // Tarjeta agregada de nuevos ciclistas (solo cuentas reales)
    newRiders_nearby_one: 'Un nuevo ciclista se unió cerca de ti esta semana',
    newRiders_nearby_other: '{{count}} nuevos ciclistas se unieron cerca de ti esta semana',
    newRiders_region_one: 'Un nuevo ciclista se unió en tu región esta semana',
    newRiders_region_other: '{{count}} nuevos ciclistas se unieron en tu región esta semana',
    newRiders_community_one: 'Un nuevo ciclista se unió a la comunidad Defensive Pedal esta semana',
    newRiders_community_other: '{{count}} nuevos ciclistas se unieron a la comunidad Defensive Pedal esta semana',
  },

  // ── Community Stats ──
  communityStats: {
    loading: 'Cargando estadísticas de la comunidad...',
    title: 'Tu comunidad ciclista',
    cyclistsIn: 'Ciclistas en',
    emptyBody:
      'Aún no se han compartido rutas cerca — sé el primer ciclista de este mapa. El feed de la comunidad de abajo sigue mostrando ciclistas de más lejos.',
    totalRides: 'Rutas totales',
    distance: 'Distancia',
    rideTime: 'Tiempo en bici',
    co2Saved: 'CO2 ahorrado',
    activeRider_one: 'ciclista activo',
    activeRider_other: 'ciclistas activos',
  },

  // ── City Heartbeat ──
  cityHeartbeat: {
    title: 'Pulso de la ciudad',
    loadFailed: 'No pudimos cargar el pulso de tu ciudad',
    cardTitle: 'Pulso de la ciudad',
    cardSub: 'Actividad en directo, tendencias, peligros y mejores ciclistas',
    todayPulse: 'PULSO DE HOY',
    allTime: 'HISTÓRICO',
    allTimeNearby: 'HISTÓRICO · CERCA DE TI',
    topContributorsCommunity: 'TOP COLABORADORES · TODA LA COMUNIDAD',
    rides: 'Rutas',
    ridesStarted: 'Rutas',
    sharedRides: 'Rutas compartidas',
    routesPlanned: 'Rutas planificadas',
    orbRiders: 'ciclistas',
    accounts: 'Cuentas',
    ridersToday: 'En bici o patinete hoy (est.)',
    cityEstimateSourceScooter: 'Estimación, no un recuento: {{percent}}% de los viajes en bici o patinete (Master Plan Velo, {{year}}) sobre {{population}} habitantes.',
    cyclistsToday: 'Ciclistas hoy (est.)',
    conditions_storm: 'Muchos menos hoy — tormenta',
    conditions_rain: 'Menos hoy — lluvia probable',
    conditions_cold: 'Menos hoy — frío',
    conditions_hot: 'Menos hoy — calor',
    conditions_windy: 'Algo menos hoy — viento',
    conditions_ideal: 'Más hoy — buen tiempo para pedalear',
    cityEstimateTitle: 'CICLISMO EN {{city}}',
    cyclistsDaily: 'Ciclistas al día (est.)',
    cityEstimateSource: 'Estimación, no un recuento: {{percent}}% de los viajes en bici (Banco Mundial, datos {{year}}) sobre {{population}} habitantes.',
    cyclingEuropeTitle: 'CICLISMO EN EUROPA',
    cyclingEuropeSub_one: 'Contados por los contadores de 1 ciudad esta semana — no rutas de la app',
    cyclingEuropeSub_other: 'Contados por los contadores de {{cities}} ciudades esta semana — no rutas de la app',
    cyclistsCounted: 'Ciclistas contados',
    countersReporting: 'Contadores',
    networkTitle: 'LA RED DE SEGURIDAD',
    networkSub: 'Por donde circula cada ciclista de aquí',
    roadsScored: 'Vías evaluadas',
    hazardsMapped: 'Peligros en el mapa',
    countriesCovered: 'Países',
    daysRidden: 'Días pedaleados',
    distance: 'Distancia',
    co2Saved: 'CO2 ahorrado',
    donated: 'Donado',
    totalRides: 'Rutas totales',
    riders: 'Ciclistas',
    hazardHotspots: 'PUNTOS PELIGROSOS',
    hazardHotspotsSub: 'Los más reportados en los últimos 7 días',
    topContributors: 'TOP COLABORADORES',
    loading: 'Cargando pulso...',
    noData: 'No hay datos disponibles',
    riderToday_one: 'ciclista activo hoy',
    riderToday_other: 'ciclistas activos hoy',
    // ── Etiquetas honestas de ventana/alcance ──
    cityFallback: 'tu ciudad',
    pulse_today_nearby: 'Hoy en {{city}}',
    pulse_week_nearby: 'Esta semana en {{city}}',
    pulse_month_nearby: 'Este mes en {{city}}',
    pulse_today_region: 'Hoy en tu región',
    pulse_week_region: 'Esta semana en tu región',
    pulse_month_region: 'Este mes en tu región',
    pulse_today_community: 'Hoy en toda la comunidad Defensive Pedal',
    pulse_week_community: 'Esta semana en toda la comunidad Defensive Pedal',
    pulse_month_community: 'Este mes en toda la comunidad Defensive Pedal',
    scopeRegionTitle: 'Tu región',
    scopeCommunityTitle: 'Comunidad Defensive Pedal',
    activeRiders_today_one: 'ciclista activo hoy',
    activeRiders_today_other: 'ciclistas activos hoy',
    activeRiders_week_one: 'ciclista activo esta semana',
    activeRiders_week_other: 'ciclistas activos esta semana',
    activeRiders_month_one: 'ciclista activo este mes',
    activeRiders_month_other: 'ciclistas activos este mes',
    chartTitleDaily: 'Últimos 7 días',
    chartTitleWeekly: 'Últimas 4 semanas',
    communityAllTime: 'Total de la comunidad',
    communityAllTimeSub: 'En toda la comunidad Defensive Pedal',
  },

  // ── Feed Card ──
  feedCard: {
    now: 'ahora',
    minsAgo: 'hace {{count}}m',
    hoursAgo: 'hace {{count}}h',
    daysAgo: 'hace {{count}}d',
    // Moderation menu (long-press) — compliance plan item 7
    moderationMenu: '¿Qué quieres hacer?',
    report: 'Reportar',
    blockUser: 'Bloquear usuario',
    longPressHint: 'Mantén pulsado para más opciones',
    blockConfirmTitle: '¿Bloquear a este usuario?',
    blockConfirmMessage: 'Ya no verás publicaciones, peligros ni comentarios de {{name}}.',
    blockErrorTitle: 'No se pudo bloquear',
    blockErrorMessage: 'Por favor, inténtalo de nuevo en un momento.',
    hiddenAfterAction: 'Oculto — refresca el feed para ver otras publicaciones.',
    commentsTitle: 'Comentarios',
    commentPlaceholder: 'Añade un comentario…',
    sendComment: 'Enviar comentario',
    dismissComments: 'Cerrar comentarios',
    noCommentsYet: 'Aún no hay comentarios. ¡Sé el primero!',
    openComments: 'Abrir comentarios ({{count}})',
    viewMoreComments_one: 'Ver 1 comentario anterior',
    viewMoreComments_other: 'Ver {{count}} comentarios anteriores',
  },

  // ── Hazard Reporting ──
  // ── Sesizări (traspaso a civia.ro, solo Rumanía) ──
  // Solo texto de interfaz. La petición se compone en
  // packages/core/src/sesizare.ts y siempre está en rumano.
  sesizare: {
    cta: 'Reclamar al ayuntamiento',
    ctaShort: 'Reclamación',
    subtitle: 'Envía el problema a la autoridad responsable, a través de Civia',
    alreadyEscalated: 'Ya enviaste esta reclamación',
    othersEscalated_one: 'Un ciclista ya lo ha reclamado al ayuntamiento',
    othersEscalated_other: '{{count}} ciclistas ya lo han reclamado al ayuntamiento',
    copiedTitle: 'Texto de la reclamación copiado',
    copiedBody:
      'Pégalo en el campo "Descriere" de Civia. Añadir una foto allí aumenta mucho las opciones de que lo arreglen.',
    openCivia: 'Abrir Civia',
    clipboardFailedTitle: 'No se pudo copiar el texto',
    clipboardFailedBody: 'Inténtalo de nuevo en un momento.',
    postRideTitle: '¿Lo llevas más lejos?',
    postRideBody:
      'Has reportado peligros en esta ruta. Algunos pueden enviarse a la autoridad responsable.',
    dashboardLabel: 'Reclamaciones al ayuntamiento',
    dashboardSub: 'Enviadas a la autoridad responsable, a través de Civia',
  },
  hazard: {
    title: 'Reportar peligro',
    parkedCar: 'Coche aparcado',
    blockedLane: 'Carril bloqueado',
    pothole: 'Bache',
    aggroDogs: 'Perros agresivos',
    aggroTraffic: 'Tráfico agresivo',
    other: 'Otro',
    reported: '¡Reportado! Otros ciclistas serán avisados.',
    stillThere: '¿Sigue ahí?',
    stillHereNow: '¿Todavía está aquí?',
    warning: 'Peligro por delante',
    confirm: 'Confirmar',
    deny: 'Ya no está',
    upvoteLabel: 'Confirmar peligro',
    upvoteHint: 'Confirma que el peligro sigue presente',
    downvoteLabel: 'Marcar como resuelto',
    downvoteHint: 'Reporta que el peligro ya no está',
    // Quick-report sheet during navigation
    describeTitle: 'Describe el peligro',
    describeSubtitle: 'Opcional — una nota corta ayuda a otros ciclistas.',
    describePlaceholder: 'p. ej. perro suelto, cristales, valla cerrada…',
    describeA11y: 'Descripción del peligro, opcional',
    describeA11yHint: 'Escribe una descripción corta o déjalo en blanco',
    reportShort: 'Reportar',
    backToTypes: 'Atrás',
    backToTypesA11y: 'Volver a los tipos de peligro',
    dismissPickerA11y: 'Cerrar el selector de peligros',
    cancelReportA11y: 'Cancelar reporte',
    reportItemA11y: 'Reportar {{label}}',
    communityScore: 'Puntuación de la comunidad',
    reportedAgo: 'Reportado {{time}}',
    lastConfirmedAgo: 'Última confirmación {{time}}',
    importedFrom: 'Reportado vía {{source}}',
    // Peligros permanentes (migración 202609020001). Se marcan al reportar; sin
    // TTL, solo desaparecen cuando {{count}} ciclistas los descartan.
    permanentLabel: 'Este peligro es permanente',
    permanentHint: 'No caduca por sí solo — solo desaparece si {{count}} ciclistas dicen que ya no está.',
    permanentBadge: 'Permanente',
    permanentExplainer: 'Sin fecha de caducidad. Permanece en el mapa hasta que {{count}} ciclistas lo descarten — {{count_current}} hasta ahora.',
    confirms: 'Confirmaciones',
    dismisses: 'Descartes',
    types: {
      illegally_parked_car: 'Coche aparcado',
      blocked_bike_lane: 'Carril bici bloqueado',
      missing_bike_lane: 'Falta carril bici',
      pothole: 'Bache',
      poor_surface: 'Mal firme',
      narrow_street: 'Calle estrecha',
      dangerous_intersection: 'Cruce peligroso',
      aggro_dogs: 'Perros agresivos',
      aggressive_traffic: 'Tráfico agresivo',
      other: 'Peligro',
    },
  },

  // ── Search ──
  search: {
    searching: 'Buscando lugares…',
    noMatches: 'Aún no hay resultados. Sigue escribiendo o prueba un punto de referencia cercano.',
    recent: 'Recientes',
    currentLocation: 'Ubicación actual',
    home: 'Casa',
    work: 'Trabajo',
    setHome: 'Guardar dirección de casa',
    setWork: 'Guardar dirección de trabajo',
    saveAs: 'Guardar este lugar como…',
    saveAsHome: 'Establecer como Casa',
    saveAsWork: 'Establecer como Trabajo',
    savedPlaces: 'Lugares guardados',
    selectA11y: 'Seleccionar {{name}}',
    saveHintA11y: 'Mantén pulsado para guardar como Casa o Trabajo',
  },

  // ── Weather ──
  weather: {
    feelsLike: 'Sensación',
    wind: 'Viento',
    aqi: 'Calidad del aire',
    precipitation: 'Precipitación',
    loadingWeather: 'Cargando tiempo...',
  },

  // ── Weather Warning Modal ──
  weatherWarning: {
    title: 'Aviso meteorológico',
    subtitle: 'Mantente alerta y rueda con precaución.',
    startAnyway: 'Empezar de todos modos',
    rain: 'Alta probabilidad de lluvia más tarde ({{percent}}%) — lleva ropa de lluvia y rueda con precaución',
    cold: 'Frío: {{temp}}°C — abrígate y rueda con precaución',
    hot: 'Calor: {{temp}}°C — hidrátate y rueda con precaución',
    warmingUp: 'Subiendo de temperatura más tarde: {{min}}°C → {{max}}°C — sal con capas ligeras y lleva agua',
    coolingDown: 'Bajando de temperatura más tarde: {{max}}°C → {{min}}°C — lleva una capa y rueda con precaución',
    tempSwing: 'Gran variación de temperatura: {{min}}°C – {{max}}°C — viste por capas y rueda con precaución',
    windBreezy: 'Brisa: {{wind}} — rueda con precaución',
    windBreezyGust: 'Brisa: {{wind}}, rachas hasta {{gust}} — rueda con precaución',
    windStrong: 'Viento fuerte: {{wind}} — rueda con precaución',
    windStrongGust: 'Viento fuerte: {{wind}}, rachas hasta {{gust}} — rueda con precaución',
    windHazardous: 'Viento peligroso: {{wind}} — precaución extrema, prefiere rutas resguardadas',
    windHazardousGust: 'Viento peligroso: {{wind}}, rachas hasta {{gust}} — precaución extrema, prefiere rutas resguardadas',
    airQualityPoor: 'Calidad del aire mala (AQI {{aqi}}) — limita el esfuerzo y rueda con precaución',
    airQualityModerate: 'Calidad del aire moderada (AQI {{aqi}}) — los grupos sensibles deben rodar con precaución',
    pm25: 'Partículas finas elevadas: PM2.5 {{pm}} μg/m³ — rueda con precaución',
  },

  // ── Stats Dashboard ──
  stats: {
    week: 'Semana',
    month: 'Mes',
    allTime: 'Histórico',
    trips: 'Rutas',
    distance: 'Distancia',
    duration: 'Duración',
    co2Saved: 'CO2 ahorrado',
    caloriesBurned: 'Calorías',
    avgSpeed: 'Velocidad media',
  },

  // ── Streak ──
  streak: {
    dayStreak: 'Racha de {{count}} días',
    frozen: 'Congelada',
    freezeReady: 'Congelación disponible',
    freezeUsed: 'Congelación usada',
    keepItUp: '¡Sigue así!',
    startChain: '¡Empieza tu racha hoy!',
    longest: 'Más larga:',
    a11yLabel: 'Racha de {{days}} días, {{tier}}',
    dormant: 'Inactiva',
  },

  // ── Streak tier names ──
  streakTier: {
    kindling: 'Yesca',
    spark: 'Chispa',
    commute: 'Hábito de trayecto',
    endurance: 'Medio maratón sobre ruedas',
    binary: 'Año binario',
    century: 'Centenario',
    legend: 'Leyenda',
  },

  // ── Guardian Tiers ──
  guardian: {
    reporter: 'Reportero',
    watchdog: 'Vigilante',
    sentinel: 'Centinela',
    guardianAngel: 'Ángel guardián',
    maxTier: '¡Nivel máximo alcanzado!',
  },

  // ── Auth ──
  auth: {
    forgotPassword: '¿Olvidaste la contraseña?',
    resetEnterEmailFirst: 'Introduce primero tu email arriba y vuelve a tocar "¿Olvidaste la contraseña?".',
    resetEmailSent: 'Email de restablecimiento enviado — revisa tu bandeja y abre el enlace en este teléfono.',
    resetPasswordTitle: 'Establece una nueva contraseña',
    resetPasswordSubtitle: 'Elige una nueva contraseña para tu cuenta.',
    resetPasswordNew: 'Nueva contraseña',
    resetPasswordConfirm: 'Confirmar contraseña',
    resetPasswordSubmit: 'Guardar nueva contraseña',
    resetPasswordTooShort: 'La contraseña debe tener al menos 6 caracteres.',
    resetPasswordMismatch: 'Las contraseñas no coinciden.',
    resetPasswordDoneTitle: 'Contraseña actualizada',
    resetPasswordDoneBody: 'Has iniciado sesión con tu nueva contraseña.',
    resetPasswordContinue: 'Continuar',
    resetPasswordExpiredTitle: 'Enlace caducado',
    resetPasswordExpiredBody: 'Este enlace de restablecimiento ya no es válido. Solicita uno nuevo desde la pantalla de inicio de sesión.',
    resetPasswordBackToSignIn: 'Volver a iniciar sesión',
    signIn: 'Iniciar sesión',
    signUp: 'Crear cuenta',
    email: 'Correo electrónico',
    password: 'Contraseña',
    submit: 'Enviar',
    enterBoth: 'Introduce correo y contraseña.',
    signedInSuccess: 'Sesión iniciada correctamente.',
    accountCreated: 'Revisa tu bandeja de entrada — te enviamos un enlace de confirmación. Ábrelo en un teléfono con Defensive Pedal instalado.',
    resendConfirmation: 'Reenviar email de confirmación',
    confirmResent: 'Email de confirmación enviado — revisa tu bandeja de entrada.',
    authFailed: 'Error de autenticación.',
    devSession: 'Sesión de desarrollador activada para',
    devSignInFailed: 'Error en el inicio de sesión de desarrollador.',
    googleClockSkew:
      'No pudimos iniciar tu sesión con Google porque la fecha y la hora del dispositivo parecen incorrectas. Abre Ajustes › General › Fecha y hora y activa «Ajustar automáticamente», luego inténtalo de nuevo. También puedes usar Iniciar sesión con Apple o correo electrónico.',
    googleUnavailable: 'El inicio de sesión con Google no está disponible ahora. Usa Iniciar sesión con Apple o correo electrónico.',
    googleGenericError: 'No pudimos iniciar tu sesión con Google. Inténtalo de nuevo o usa Iniciar sesión con Apple o correo electrónico.',
    clockSkewBanner:
      'El reloj del dispositivo parece desfasado unos {{minutes}} min. Esto puede bloquear el inicio de sesión con Google: activa la fecha y hora automáticas en Ajustes.',
  },

  // ── Legal ──
  legal: {
    signupAgreePrefix: 'Al continuar aceptas nuestros ',
    termsOfService: 'Términos del Servicio',
    signupAgreeAnd: ' y la ',
    privacyPolicy: 'Política de Privacidad',
    signupAgreeSuffix: '.',
  },

  // ── Settings ──
  settings: {
    title: 'Menú',
    subtitle: 'Centro de control',
    signedInAccount: 'Cuenta activa',
    active: 'Activa',
    offline: 'Modo solo local',
    noSession: 'No hay sesión de cuenta activa.',
    provider: 'Proveedor:',
    sync: 'Sincronización:',
    syncReady: 'Listo para rutas, peligros y comentarios',
    account: 'Cuenta',
    accountSub: 'Inicia sesión, crea una cuenta o ciérrala',
    offlineMaps: 'Mapas sin conexión',
    offlineMapsSub: 'Descarga mapas para uso sin conexión',
    diagnostics: 'Diagnóstico',
    diagnosticsSub: 'Comprueba la conectividad y el estado de los servicios',
    welcomeFlow: 'Flujo de bienvenida',
    welcomeFlowSub: 'Reinicia la experiencia de introducción',
    helpFaq: 'Ayuda y FAQ',
    helpFaqSub: 'Respuestas a preguntas frecuentes',
    privacyPolicy: 'Política de Privacidad',
    privacyPolicySub: 'Ver nuestra política de privacidad',
    environment: 'Entorno:',
    variant: 'Variante:',
  },

  // ── Diagnostics ──
  diag: {
    title: 'Diagnóstico',
    subtitle: 'Estado de la app y conectividad',
    checking: 'Comprobando...',
    passed: 'Correcto',
    failed: 'Fallido',
    timeout: 'Tiempo agotado',
  },

  // ── Offline Maps ──
  offlineMaps: {
    title: 'Mapas sin conexión',
    subtitle: 'Descarga mapas para uso sin conexión',
    download: 'Descargar',
    downloading: 'Descargando...',
    downloaded: 'Descargado',
    delete: 'Eliminar',
  },

  // ── Onboarding ──
  onboarding: {
    welcome: 'Bienvenido a Defensive Pedal',
    createAccount: 'Crear cuenta',
    skip: 'Omitir por ahora',
    // Paso de tipo de bicicleta (tras el registro).
    bikeTypeTitle: '¿Qué bicicleta usas?',
    bikeTypeSubtitle: 'Ajustaremos tus rutas a tu bicicleta.',
    bikeTypeContinue: 'Continuar',
    bikeRoadDesc: 'Cubiertas finas, mejor sobre asfalto liso',
    bikeCityDesc: 'Trayectos diarios y recados',
    bikeMountainDesc: 'Hecha para senderos, grava y terreno irregular',
    bikeEbikeDesc: 'Con pedaleo asistido o eléctrica',
    bikeRecumbentDesc: 'Postura reclinada, cerca del suelo',
    bikeOtherDesc: 'Otra — plegable, de carga, triciclo',
    bikeConsequenceNone: 'Elige una y ajustaremos tu preferencia de superficie. Puedes cambiarla cuando quieras en Perfil.',
    bikeConsequencePaved: 'Activaremos «Evitar caminos sin asfaltar» para que tus rutas sigan por asfalto. Puedes cambiarlo cuando quieras en Perfil.',
    bikeConsequenceUnpaved: 'Desactivaremos «Evitar caminos sin asfaltar» para que los senderos y la grava sigan disponibles. Puedes cambiarlo cuando quieras en Perfil.',
    bikeConsequenceNeutral: 'Dejaremos tu ajuste «Evitar caminos sin asfaltar» como está. Puedes cambiarlo cuando quieras en Perfil.',

    getStarted: 'Empezar',
    oneMoreThing: 'Una cosa más',
    chooseUsername: 'Elige un nombre de usuario',
    usernameSub: 'Así te verán los demás ciclistas.',
    usernamePlaceholder: 'tu_usuario',
    locationEnabled: 'Ubicación activada',
    regionConfirmed: 'País confirmado',
    accountCreated: 'Cuenta creada',
    // -- Wired screen copy (review 2026-06-12 i18n) --
    skipShort: 'Omitir',
    a11ySkip: 'Omitir la introducción',
    a11yBack: 'Atrás',
    continue: 'Continuar',
    locationTitle: 'Rutas que evitan las carreteras peligrosas',
    locationSubtitle: 'Usamos tu ubicación para encontrar las rutas ciclistas más seguras cerca de ti, evaluadas con datos reales de riesgo vial.',
    featureStat: '2,1× más seguro que Google Maps',
    featureStatSub: 'Medido en más de 10.000 rutas con puntuación de riesgo en Rumanía y España',
    featureRoutes: 'Rutas centradas en la seguridad',
    featureRoutesSub: 'Datos reales de riesgo vial en 32 países europeos',
    featureHazards: 'Alertas de peligro',
    featureHazardsSub: 'Peligros reportados por la comunidad cerca de tu ruta',
    locationDenied: 'Ubicación no disponible. Aún puedes explorar rutas buscando una ciudad.',
    continueWithoutLocation: 'Continuar sin ubicación',
    openSettings: 'Abrir Ajustes',
    enableLocation: 'Continuar',
    usernameCommunitySub: 'Así te verán otros ciclistas en la comunidad.',
    usernameHint: 'Mínimo 3 caracteres. Solo letras, números y guion bajo.',
    almostThere: 'Ya casi',
    createYourAccount: 'Crea tu cuenta',
    signupSubMandatory:
      'Las alertas de peligro solo funcionan porque cada aviso viene de un ciclista real. Tu cuenta gratuita mantiene esos datos fiables — y guarda tus rutas, rachas e insignias en cualquier teléfono.',
    signupSub: 'Tus rutas, rachas e insignias se conservan. Sincronízalas y únete a la comunidad. Siempre puedes hacerlo más tarde.',
    percentComplete: '{{percent}}% completado',
    continueWithGoogle: 'Continuar con Google',
    or: 'o',
    signUpWithEmail: 'Regístrate con email',
    maybeLater: 'Quizás más tarde',
    a11yGoogle: 'Iniciar sesión con Google',
    // -- Mandatory registration (2026-07-24): Google dominant, email demoted,
    //    guest path removed. The sub copy is `signupSubMandatory` above. --
    signupPromptTitle: 'Únete a los ciclistas que hacen tu ciudad más segura',
    useEmailInstead: 'Prefiero usar el correo',
    trustMicroline: 'Gratis para siempre. Tus rutas son privadas por defecto.',
    telemetryNotice:
      'Recopilamos informes anónimos de fallos y analíticas anónimas de producto (sin trazas GPS) para mantener la app estable y mejorarla. Desactiva cualquiera cuando quieras en Perfil › Privacidad.',
    // -- Region gate (disponibilidad global, 2026-07-12) --
    regionChecking: 'Comprobando la disponibilidad en tu zona…',
    regionPickerTitle: '¿Dónde pedaleas?',
    regionPickerSubtitle:
      'No pudimos detectar tu país automáticamente. Elígelo abajo para mostrarte qué está disponible.',
    regionSearchPlaceholder: 'Buscar países',
    regionNoResults: 'Ningún país coincide con tu búsqueda',
    regionUnavailableTitle: 'Aún no estamos en {{country}}',
    regionUnavailableSubtitle:
      'Defensive Pedal cubre actualmente la UE, el EEE, Suiza y el Reino Unido. Déjanos tu email y te avisaremos en cuanto lancemos en {{country}}.',
    regionEmailPlaceholder: 'tu@ejemplo.com',
    regionEmailConsent: 'Solo te escribiremos sobre la disponibilidad en tu país — nada más.',
    regionNotifyMe: 'Avisadme',
    regionEmailInvalid: 'Introduce una dirección de email válida.',
    regionSubmitFailed: 'No pudimos guardar tu email. Comprueba tu conexión e inténtalo de nuevo.',
    regionSubmitted: '¡Estás en la lista! Te escribiremos cuando lancemos en {{country}}.',
    regionContinueAnyway: 'Continuar de todos modos',
    regionContinueAnywayHint:
      'Puedes seguir planificando rutas ciclistas estándar, pero el enrutamiento seguro aún no está disponible en tu país y los datos de peligros de la comunidad son limitados.',
    regionChangeCountry: 'Cambiar país',
  },

  // ── Impact ──
  impact: {
    title: 'Tu impacto',
    co2Saved: 'CO2 ahorrado',
    moneySaved: 'Dinero ahorrado',
    hazardsReported: 'Peligros reportados',
    equivalent: 'equivalente a',
    // Post-ride ImpactSummaryCard
    thisRideImpact: 'Impacto de esta ruta',
    badgesEarnedTitle: 'Insignias conseguidas',
    viewAllAchievements: 'Ver todos los logros >',
    xpEarnedTitle: 'XP ganada',
    xpTotal: 'Total',
    legendMaxRank: 'Leyenda — Rango máximo',
    dayStreakShort: 'días de racha',
    streakDayA11y: 'Día {{count}} de racha',
    lifeEarnedFmt: '+{{time}} de vida ganada',
    donatedSecondsFmt: '+{{seconds}}s donados a la ciudad',
    caloriesBurned: 'Calorías quemadas',
  },

  // ── FAQ ──
  faq: {
    title: 'Preguntas frecuentes',
    subtitle: 'Respuestas a las preguntas habituales',
  },

  // ── Daily Safety Quiz ──
  quiz: {
    title: 'Cuestionario diario',
    loadFailed: 'No se pudo cargar el cuestionario',
    retry: 'Reintentar',
    correct: '¡Correcto!',
    notQuite: 'No del todo',
    done: 'Hecho',
    streakMaintained: '¡Racha mantenida! Sigue así.',
  },

  // ── Achievements / Trophy Case ──
  achievements: {
    screenTitle: 'Logros',
    loadFailed: 'No pudimos cargar tus insignias',
    loadFailedSub: 'Revisa tu conexión e inténtalo de nuevo',
    holoA11y: 'Insignia holográfica',
    yourAchievements: 'TUS LOGROS',
    moreToUnlock: '+ {{count}} por desbloquear',
    emptyAllTitle: 'Aún ninguna insignia — vamos a por la primera.',
    emptyCategory: 'Aún no hay insignias en esta categoría. ¡Sigue pedaleando!',
    timeJustNow: 'ahora mismo',
    timeMinsAgo: 'hace {{count}}m',
    timeHoursAgo: 'hace {{count}}h',
    timeDaysAgo: 'hace {{count}}d',
    tabAll: 'Todas',
    tabFirsts: 'Primeras',
    tabRiding: 'Rodar',
    tabConsistency: 'Constancia',
    tabImpact: 'Impacto',
    tabSafety: 'Seguridad',
    tabCommunity: 'Comunidad',
    tabExplore: 'Explorar',
    tabEvents: 'Eventos',
    secretName: '???',
    secretFlavor: 'Esta insignia es un misterio. Sigue pedaleando para descubrirla.',
    rarityLine: 'Solo el {{percent}}% de los ciclistas tiene esta insignia',
    earnedDate: 'Conseguida el {{date}}',
    share: 'Compartir',
    progressA11y: '{{current}} de {{target}} {{unit}}, {{percent}}% completado',
  },

  // ── Microlives ──
  microlives: {
    lifeEarned: 'de vida ganada',
    donatedToCity: 'donada a tu ciudad',
    timeBank: 'Banco de Tiempo',
    personalLifeGained: 'Vida ganada en esta ruta',
    communityDonation: 'Donada a la comunidad',
  },

  // ── Trip Comparison ──
  compare: {
    title: 'Comparar rutas',
    select: 'Comparar',
    selectTrips: 'Selecciona 2 rutas para comparar',
    cancel: 'Cancelar',
    compare: 'Comparar seleccionadas',
    distance: 'Distancia',
    duration: 'Duración',
    avgSpeed: 'Velocidad media',
    co2Saved: 'CO2 ahorrado',
    mode: 'Modo',
    date: 'Fecha',
    trip1: 'Ruta 1',
    trip2: 'Ruta 2',
    faster: 'más rápida',
    slower: 'más lenta',
    longer: 'más larga',
    shorter: 'más corta',
  },

  // ── User Profile ──
  userProfile: {
    loading: 'Cargando...',
    notFound: 'Usuario no encontrado',
  },

  // ── Leaderboard ──
  leaderboard: {
    emptyStateWidened: 'Nada clasificado en este periodo — prueba uno más largo',
    scope_region: 'Aún nadie cerca — clasificación en {{distance}}',
    scope_community: 'Aún nadie cerca — clasificación de toda la comunidad',
    title: 'Clasificación del barrio',
    loadFailed: 'No pudimos cargar la clasificación. Desliza para actualizar o inténtalo de nuevo.',
    tabs: {
      co2: 'CO2 ahorrado',
      hazards: 'Peligros reportados',
    },
    periods: {
      week: 'Esta semana',
      month: 'Este mes',
      all: 'Histórico',
    },
    rank: 'Puesto',
    champion: 'Campeón',
    weeklyChampion: 'Campeón de la semana',
    ghostRankSeparator: 'Tu puesto',
    emptyState: 'Aún no hay actividad en tu zona',
    metricUnit: {
      co2: 'kg',
      hazards: 'reportados',
    },
    rankChange: {
      up: 'sube',
      down: 'baja',
      new: 'nuevo',
    },
    podium: {
      gold: '1º',
      silver: '2º',
      bronze: '3º',
    },
  },

  // ── Mia Persona Copy ──
  mia: {
    detection: {
      headline: 'Hemos visto que aún no has montado — no pasa nada.',
      body: 'Mucha gente descarga apps de ciclismo antes de sentirse lista para salir. Si quieres, podemos empezar con rutas más cortas y fáciles por las calles más tranquilas cerca de ti.',
      accept: 'Sí, empieza fácil',
      decline: 'No gracias, montaré cuando esté listo',
    },
    ride: {
      summary: {
        headline: '¡Lo lograste! Otra ruta más en tu historial.',
        hazards: 'Te cuidamos en {{count}} puntos complicados — y los pasaste todos sin problema.',
      },
    },
    route: {
      preview: {
        safetyLabel: '¿Qué tan segura es esta ruta? Muy segura.',
        safetyLabelModerate: 'Mayormente segura, con una sección más concurrida.',
        moderateWarning: 'Atención: {{street}} está un poco más concurrida — pero tiene carril bici. Te avisaremos 200m antes.',
        moderateWarningNoBikeLane: 'Atención: {{street}} está un poco más concurrida. Mantente en el carril derecho y todo irá bien.',
        allCalm: 'Cada calle de esta ruta es una calle tranquila.',
        whatToExpect: 'Qué esperar',
      },
    },
    planning: {
      emptyState: 'Tu bici te espera. Tu primera ruta son solo 5 minutos por calles tranquilas. Miles de personas como tú empezaron justo aquí.',
      firstRoute: 'Aquí tienes tu primera ruta — 5 minutos por las calles más tranquilas cerca de ti.',
      seeMyRoute: 'Ver mi primera ruta',
      autoRouteButton: 'Ver mi ruta',
      destinationIntro: '¿A dónde te gustaría ir en bici?',
    },
    navigation: {
      segmentEntry: 'Calle más concurrida por delante — carril bici a tu derecha. Ya has pasado por esto antes.',
      segmentEntryNoBikeLane: 'Calle más concurrida por delante. Mantente firme — puedes con esto.',
      segmentExit: '¡Bien hecho! Pasaste esa sección perfectamente.',
      segmentExitFirst: '¡Increíble! Acabas de pasar tu primera sección concurrida.',
    },
    postRide: {
      courageTitle: 'Tu valentía',
      segmentsHandled: 'Segmentos moderados superados: {{handled}}/{{total}}',
      segmentsHandledAll: '¡Todos los segmentos moderados superados!',
      levelProgress: '{{rides}}/{{needed}} rutas para el nivel {{level}}',
      courageLabel: 'Has superado {{handled}} de {{total}} segmentos moderados. {{rides}} de {{needed}} rutas para el siguiente nivel.',
    },
    notifications: {
      firstRideNudge: 'Tu primera ruta está lista — solo 5 minutos por calles tranquilas cerca de casa. Este fin de semana podría ser el inicio de algo grande.',
      postFirstRide: 'Ayer montaste por primera vez. ¿Recuerdas lo bien que sentó? Otra ruta corta te está esperando.',
      levelUpAvailable: '¡Estás a una ruta de desbloquear {{nextLevelName}}! Listo cuando lo estés.',
      weatherInvite: 'Tiempo perfecto para ir en bici este fin de semana. ¿Una ruta corta por calles tranquilas?',
      milestoneApproaching: 'Solo te queda {{ridesLeft}} ruta para llegar a {{nextLevelName}} — ya casi estás.',
      lapsedReengagement: 'Hace tiempo que no nos vemos — no pasa nada. Tu ruta sigue aquí cuando estés listo. Sin presión.',
    },
    levelUp: {
      '1to2': {
        title: 'Nivel 2: Explorador del barrio',
        subtitle: 'Diste la primera pedalada. Todo empieza aquí.',
        shareText: 'Acabo de completar mi primera ruta en bici con Defensive Pedal. El camino de nervioso a seguro empieza con una ruta.',
      },
      '2to3': {
        title: 'Nivel 3: Ciclista de cafetería',
        subtitle: 'Has explorado tu barrio. Ahora monta HACIA un sitio.',
        shareText: '¡3 rutas hechas! Pasé de tener miedo a montar a explorar mi barrio sobre dos ruedas.',
      },
      '3to4': {
        title: 'Nivel 4: Navegante urbano',
        subtitle: 'Ahora montas hacia destinos. La ciudad entera se abre.',
        shareText: '5 rutas completadas y ahora monto hacia destinos reales.',
      },
      '4to5': {
        title: 'Ciclista seguro',
        subtitle: 'ERES ciclista. Esto es lo que eres ahora.',
        shareText: 'Lo conseguí. De demasiado nervioso para montar a ciclista seguro. {{totalRides}} rutas, {{totalKm}} km.',
        testimonialPrompt: '¿Qué le dirías a alguien que está nervioso con el ciclismo?',
        testimonialAlt: '¿Qué te sorprendió más de tus primeras rutas?',
      },
    },
    journey: {
      title: 'Tu viaje ciclista',
      progressBar: 'Nivel {{level}}: {{name}}',
      progressRides: '{{rides}}/{{needed}} rutas',
      levelNames: {
        '1': 'Primera pedalada',
        '2': 'Explorador del barrio',
        '3': 'Ciclista de cafetería',
        '4': 'Navegante urbano',
        '5': 'Ciclista seguro',
      },
      unlockPreviews: {
        '2': 'Desbloquea rutas más largas',
        '3': 'Desbloquea ir hacia destinos',
        '4': 'Desbloquea preferencias de ruta',
        '5': 'Experiencia ciclista completa',
      },
      conquered: {
        '1': 'Calles residenciales tranquilas',
        '2': 'Segmentos con tráfico moderado',
        '3': 'Pedalear hacia destinos',
        '4': 'Navegación urbana completa',
      },
      completed: 'Viaje completado',
      graduationBadge: 'Ciclista seguro',
      shareProgress: 'Comparte tu progreso',
      writeStory: 'Escribe tu historia ciclista',
      skipAhead: 'Saltar — estoy listo para la experiencia completa',
      skipAheadSub: 'Sal del viaje guiado y desbloquea todas las funciones',
    },
    errors: {
      noSafeRoute: 'No hemos encontrado una ruta suficientemente segura para tu nivel actual. Prueba un destino más cercano o vuelve más tarde.',
    },
  },

  // ── Share (image-based) ──
  share: {
    shareRide: 'Compartir esta ruta',
    shareLevelUp: 'Compartir',
  },

  // ── City Suggestions ──
  citySuggestion: {
    fabLabel: 'Sugerir una mejora',
    fabLabelActive: 'Cancelar sugerencia',
    fabHint: 'Coloca un pin en el mapa y dinos qué mejoraría este sitio',
    crosshairHelper: 'Mueve el mapa al sitio que quieres señalar',
    footerConfirm: 'Añadir sugerencia aquí',
    footerCancel: 'Cancelar',
    modalTitle: 'Sugerir una mejora',
    modalSubtitle: 'Dinos qué hay mal aquí, o qué lo haría más seguro o mejor.',
    modalPlaceholder: 'p. ej. Hace falta un carril bici protegido — los autobuses pasan demasiado cerca en hora punta',
    modalSubmit: 'Enviar',
    modalSubmitting: 'Enviando…',
    modalCancel: 'Cancelar',
    minLengthHint: 'Añade un poco más de detalle',
    toastSuccess: '¡Sugerencia enviada — gracias!',
    toastOffline: 'Guardada — la enviaremos cuando vuelvas a tener conexión',
    toastError: 'No se pudo enviar — por favor inténtalo de nuevo',
  },

  // ── Map accessibility summary (screen readers) ──
  mapA11y: {
    hint: 'El mapa muestra la ruta, los peligros y los puntos de interés. Usa los controles superpuestos arriba y abajo.',
    empty: 'Vista de mapa',
    routeSummary: 'Ruta: {{distance}}, unos {{duration}}.',
    routeWithClimb: 'Ruta: {{distance}}, unos {{duration}}, ascensión de {{climb}}.',
    riskBreakdown: 'Mezcla de seguridad: {{breakdown}}.',
    hazardsOnRoute_one: '1 peligro reportado en esta ruta.',
    hazardsOnRoute_other: '{{count}} peligros reportados en esta ruta.',
    hazardUpcoming: 'Peligro por delante: {{type}}, a {{distance}}.',
    offRouteEntered: 'Fuera de ruta. Recalculando.',
    offRouteCleared: 'De vuelta en ruta.',
    navigating: 'Navegando. Quedan {{remaining}}.',
    planning: 'Vista previa de ruta.',
    historical: 'Ruta pasada. {{distance}} recorridos.',
    userLocationKnown: 'Tu ubicación se muestra en el mapa.',
  },

  // ── Ride-loss recovery banner (unsynced trip data) ──
  rideLoss: {
    title: 'Una ruta no se guardó',
    body: 'Parte de los datos de tu ruta no llegaron al servidor. Reintenta para guardarlos.',
    retry: 'Reintentar',
    dismiss: 'Descartar',
    a11yLabel:
      'Una ruta no se guardó. Reintenta para guardar los datos de tu ruta o descarta este aviso.',
  },

  // ── Impact Dashboard screen ──
  impactDashboard: {
    title: 'Tu impacto',
    loadError: 'No se pudo cargar el panel',
    timeBank: 'Banco de tiempo',
    lifeEarned: '+{{time}} de vida ganados',
    donatedToCity: '{{time}} donados a tu ciudad',
    experience: 'Experiencia',
    xpToGo: '{{tier}} · {{xp}} XP restantes',
    maxRank: 'Rango máximo alcanzado',
    lifetimeImpact: 'Impacto total',
    co2Saved: 'CO2 ahorrado',
    treesEquivalent: 'Como {{count}} árboles absorbiendo CO2 durante un año',
    moneySaved: 'Dinero ahorrado',
    ridersProtected: 'Ciclistas protegidos',
    ridersProtectedSub: 'Ciclistas protegidos por tus reportes',
    thisWeek: 'Esta semana',
    recentBadges: 'Insignias recientes',
    viewAll: 'Ver todo',
    weekRidesUnit: 'viajes',
    weekCompleted: 'Completados',
    weekCo2Saved: 'CO2 ahorrado',
    weekSaved: 'Ahorrado',
    weekHazards: 'Peligros reportados',
    weekCalories: 'Calorías quemadas',
    totalCaloriesBurned: 'Calorías quemadas',
    quizTitle: 'Cuestionario diario de seguridad',
    quizSubtext: 'Responde para mantener tu racha',
    quizA11y: 'Haz el cuestionario diario',
  },

  // ── My Shared Routes screen ──
  myShares: {
    title: 'Tus rutas compartidas',
    loadError: 'No se pudieron cargar tus compartidos. Desliza para actualizar.',
    emptyTitle: 'Aún no hay compartidos',
    emptySubtitle: 'Comparte una ruta y aparecerá aquí, junto con las vistas y los registros.',
    emptyCta: 'Planifica una ruta',
    revokedPill: 'Revocado',
    opens: 'aperturas',
    signups: 'registros',
    copyLink: 'Copiar enlace',
    copyLinkA11y: 'Copiar enlace de compartir',
    shareAgain: 'Compartir de nuevo',
    revoke: 'Revocar',
    revokeA11y: 'Revocar compartido',
    revokedOn: 'Revocado {{date}}',
    noExpiry: 'Sin caducidad',
    expiresToday: 'Caduca hoy',
    expiresTomorrow: 'Caduca mañana',
    expiresInDays: 'Caduca en {{days}} días',
    linkCopiedTitle: 'Enlace copiado',
    linkCopiedBody: 'Compártelo donde quieras.',
    copyFailedTitle: 'Error al copiar',
    copyFailedBody: 'No se pudo copiar el enlace. Inténtalo de nuevo.',
    revokeConfirmTitle: '¿Revocar este compartido?',
    revokeConfirmBody: 'Quienes abran el enlace verán una página de "ya no disponible".',
    revokeFailedTitle: 'Error al revocar',
    revokeAuthRequired: 'Inicia sesión e inténtalo de nuevo.',
    revokeNetworkError: 'Comprueba tu conexión e inténtalo de nuevo.',
  },
  premium: {
    name: 'Pedal Plus',
    badge: 'PLUS',
    sheetTitle: 'Pedal Plus',
    sheetSubtitle: 'Manten la app viva y pedalea con mas.',
    benefitRoutesTitle: 'Rutas guardadas ilimitadas',
    benefitRoutesBody: 'Guarda cada trayecto, vuelta y recado. Gratis guardas {{count}}.',
    benefitPacksTitle: 'Mapas sin conexion que se quedan',
    benefitPacksBody: 'Las descargas dejan de caducar a los {{days}} dias, y caben mas.',
    benefitHistoryTitle: 'Todo tu historial',
    benefitHistoryBody: 'Cada ruta que has hecho, no solo los ultimos {{days}} dias.',
    benefitEbikeTitle: 'Rutas para e-bike',
    benefitEbikeBody: 'Rutas y tiempos calculados para un motor, no para tus piernas.',
    benefitCoolTitle: 'Rutas frescas',
    benefitCoolBody: 'Rutas con sombra para los dias de calor.',
    planMonthly: 'Mensual',
    planAnnual: 'Anual',
    perMonth: '{{price}} / mes',
    perYear: '{{price}} / ano',
    trialNote: '{{days}} dias gratis, luego {{price}}.',
    cta: 'Empezar prueba gratis',
    ctaNoTrial: 'Suscribirse',
    restore: 'Restaurar compra',
    manage: 'Gestionar suscripcion',
    legal: 'Se renueva automaticamente hasta que la canceles. Cancela cuando quieras en la tienda.',
    activeTitle: 'Pedal Plus esta activo',
    trialActive: 'Prueba gratis - se renueva el {{date}}',
    renews: 'Se renueva el {{date}}',
    endsOn: 'Termina el {{date}}',
    billingRetry: 'Hay un problema con tu pago. Actualizalo para mantener Plus.',
    offlineNotice: 'Mostrando tu ultimo estado confirmado.',
    profileRowFree: 'Consigue Pedal Plus',
    profileRowActive: 'Pedal Plus',
    limitRoutesTitle: 'Limite de rutas guardadas',
    limitRoutesBody: 'Gratis guardas {{count}} rutas. Borra una para hacer sitio o pasate a Plus para ilimitadas.',
    limitCoursesTitle: 'Has alcanzado el límite de rutas',
    limitCoursesBody: 'Gratis puedes guardar {{count}} rutas importadas en el dispositivo. Con Pedal Plus son ilimitadas.',
    limitPacksTitle: 'Limite de mapas sin conexion',
    limitPacksBody: 'Gratis incluye {{count}} zona sin conexion. Con Plus guardas mas y dejan de caducar.',
    limitFlatTitle: 'Rutas llanas agotadas',
    limitFlatBody: 'Has usado tus {{count}} rutas llanas de este mes. Calcularemos una ruta Safe, o pasate a Plus para ilimitadas.',
    limitHistoryTitle: 'Las rutas antiguas estan ocultas',
    limitHistoryBody: 'Gratis ves los ultimos {{days}} dias. Tus rutas antiguas estan a salvo - Plus te las devuelve.',
    flatRemaining: 'Te quedan {{count}} rutas llanas este mes',
    coolUnavailable: 'Las rutas frescas aun no estan disponibles en tu pais.',
    coolRequiresPlus: 'Las rutas frescas forman parte de Pedal Plus.',
    upgrade: 'Pasate a Plus',
    notNow: 'Ahora no',
  },
};
