...
- bicycle parking markers (2026-03-24): blue circle "P" markers from Overpass API (OpenStreetMap `amenity=bicycle_parking`) now appear on all three map screens (route planning, route preview, navigation). Markers are visible at zoom level 12+. Tapping a marker shows a callout identifying it as bicycle parking. Data is fetched via `useBicycleParking` hook with 5-minute TanStack Query cache
- safe area fix (2026-03-24): replaced `SafeAreaView` from `react-native` (iOS-only) with `useSafeAreaInsets` from `react-native-safe-area-context` across MapStageScreen, NavigationScreen, and Screen components so content respects status bar and navigation buttons on Android
- voice guidance default (2026-03-24): voice guidance is now off by default (`voiceGuidanceEnabled: false`). Compact voice toggle icon (matching navigation style) added to route planning and route preview screens
- bottom sheet UX (2026-03-24): CollapsibleSheet footer (Start Navigation / Back to Planning buttons) now stays visible when sheet is collapsed; only scrollable content hides. Route planning destination selection now centers map on selected location and dismisses keyboard
