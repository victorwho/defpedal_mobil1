...
1. **Route-share slice 8b — mobile UI**: replace the `/my-shares` stub with the real screen (list + copy/revoke actions, empty state), add `AmbassadorImpactCard` organism to Impact Dashboard (consumes `GET /v1/route-shares/mine.ambassadorStats`), wire "Share activity feed" toggle to `PATCH /profile.shareConversionFeedOptin` in Profile → Display, add `RouteShareSignupFeedCard` for the new `route_share_signup` activity-feed type, add "My Shared Routes" count-badge row to Profile → Account, and the web viewer's `POST /r/:code/view` beacon hook.
2. **Cloud Run redeploy for slice 8a endpoints**: `gcloud builds submit --config cloudbuild.yaml --timeout=600 && gcloud run deploy defpedal-api --image <new> --region europe-central2 --platform managed --allow-unauthenticated`. Migration already applied; endpoints exist in code but won't be callable until redeploy.
3. **Route-share slice 5 — `past_ride` source variant**: the contract discriminator still has `past_ride` stubbed with `z.never()`. Slice 5b would replace it with `tripId` + server-side re-planning + ghost polyline. Saved variant (slice 5a) already shipped.
