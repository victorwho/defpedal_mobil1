...
1. **Route-share slice 5 — `saved` + `past_ride` source variants**: the contract already has the discriminator (`source: 'saved' | 'past_ride'`) stubbed with `z.never()` so slice 1/2 can't accidentally accept them. Slice 5 replaces the stubs with `savedRouteId` / `tripId` payload schemas, extends the API writer to resolve those into a planned-route payload (reuse the same polyline6 + riskSegments + safetyScore shape), and wires the mobile share UI so saved-routes and trip-history cards can share too. No web-viewer changes needed — the public view is already source-agnostic.
2. **Route-share slice 7 — PostHog attribution bridge + web analytics**: consume the `utm_source=share&utm_medium=web&utm_campaign=r_<code>` params already emitted by `ShareCtas.tsx` in a PostHog snippet on the web viewer, read the `dp_share_code` cookie (set by `apps/web/middleware.ts`, not HttpOnly for this reason) to correlate web session → mobile identify on first app open post-claim.
3. **Route-share slice 8 — first-view push notification**: notify the sharer via Expo Push when their share record's `view_count` increments from 0 → 1. Already has the table column + RLS; needs a Postgres trigger or server-side dispatch hook, plus a new notification category in `notifications.ts` + profile toggle.
