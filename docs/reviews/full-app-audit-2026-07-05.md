...
>
> **Deploy note:** the API-side fixes (rate limits, Sentry scrub, OSRM timeout, merge deleteUser) are in the codebase but need a Cloud Run build+deploy to go live. The SEC-1 SQL guard is ALREADY live (applied via migration), so the replay hole is closed at the data layer even before the deploy.
