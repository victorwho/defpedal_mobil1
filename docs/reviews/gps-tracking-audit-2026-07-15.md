...
1. **Save the trail on resume-prompt Discard** (P1-0) — the single change that recovers the biggest current loss bucket. Either save-by-default with `app_killed` (old behavior, deletable from History) or a three-way prompt. Ship in the v0.2.101+ line.
...
5. **Raise or adaptively thin the 2000-crumb buffer** (P1-1): e.g. thin older samples to every 2nd/4th instead of evicting, keeping endpoints — preserves ride shape at the same memory cost.
6. **Surface background-permission status** in the nav UI (P1-2) — the snapshot hook is already wired, just unread.
